import os
import re
import io
import json
import sys
import time
import shutil
import threading
import queue
import mimetypes
import hashlib
import subprocess
import tempfile
from concurrent.futures import ThreadPoolExecutor, as_completed
from datetime import datetime
from dataclasses import dataclass
from typing import List, Optional, Tuple, Set

import tkinter as tk
from tkinter import ttk, filedialog, messagebox, simpledialog

import numpy as np
from PIL import Image, ImageDraw, ImageFont

from google import genai
from google.genai import types


# ---------------------------
# Config geral
# ---------------------------

VIDEO_EXTS = {".mp4", ".mov", ".mkv", ".avi", ".webm", ".m4v"}
IMAGE_EXTS = {".jpg", ".jpeg", ".png", ".webp", ".bmp", ".tiff"}

INVALID_FILENAME_CHARS = '<>:"/\\|?*\n\r\t'


def get_app_base_dir() -> str:
    """
    Retorna a pasta "base" do app:
    - Rodando como .exe (PyInstaller): pasta do executável
    - Rodando como .py: pasta do arquivo .py
    """
    if getattr(sys, "frozen", False):
        # .exe empacotado
        return os.path.dirname(os.path.abspath(sys.executable))
    # script normal
    return os.path.dirname(os.path.abspath(__file__))


APP_BASE_DIR = get_app_base_dir()

# ✅ NOVO: tudo vai para uma pasta "cache" ao lado do script/exe
CACHE_DIR = os.path.join(APP_BASE_DIR, "cache")

CONFIG_PATH = os.path.join(CACHE_DIR, "scene_renamer_config.json")
SCENE_CACHE_PATH = os.path.join(CACHE_DIR, "scene_renamer_scene_cache.json")
SCENE_CACHE_VERSION = 1

STABLE_MATCH_CACHE_PATH = os.path.join(
    CACHE_DIR, "scene_renamer_stable_matches.json"
)
STABLE_MATCH_CACHE_VERSION = 1

LAST_RUN_STATE_PATH = os.path.join(
    CACHE_DIR, "scene_renamer_last_run.json"
)
LAST_RUN_STATE_VERSION = 1


UNDO_LAST_RUN_PATH = os.path.join(
    CACHE_DIR, "scene_renamer_undo_last_run.json"
)
UNDO_LAST_RUN_VERSION = 1

DEFAULT_UPDATE_NOTES_TEXT = """

Versão 1.1

Novidades desta versão:
- Histórico estável de nomes e matches para manter resultados mais consistentes entre execuções.
- Botão "Reprocessar pendências" para tentar novamente apenas nas cenas que ficaram sem aproveitamento.
- Repetição de cenas: a mesma cena pode ser usada em mais de uma frase.
- Limite máximo de usos por cena configurável no GUI.
- Ajuste de score para repetição de cena, reduzindo um pouco a prioridade a cada novo uso.
- Correção do nome estável quando a mesma cena é usada em frases diferentes.
- Botão "Desfazer última execução" para reverter renomeações, cópias, placeholders e relatórios da última rodada.
- Histórico de undo salvo em cache.
- Estado da última execução salvo para facilitar o reprocessamento de pendências.
- Criação de imagem verde para frases sem cena agora pode ser ligada/desligada no GUI.
- Placeholders verdes entram no undo.
- Relatórios JSON continuam sendo gerados para conferência do resultado.
- O reprocessamento de pendências agora salva e reaproveita corretamente tanto as cenas pendentes quanto as frases pendentes.
- O botão "Reprocessar pendências" agora trabalha apenas com o que realmente ficou pendente da rodada anterior, em vez de tentar casar novamente o roteiro inteiro.
- O score mínimo de aceitação no reprocessamento agora pode ficar mais frouxo a cada nova tentativa, sem alterar o valor padrão usado em "Enviar e renomear".
- Foi adicionado um fator específico de reprocessamento (`REPROCESS_MIN_ASSIGN_SCORE_FACTOR`) para controlar o quanto o critério de aceitação será reduzido a cada nova tentativa.
- O valor de `REPROCESS_MIN_ASSIGN_SCORE_FACTOR` agora pode ser configurado pelo menu superior em "Configurações" → "Configuração de Reprocessamento".
- A tentativa atual de reprocessamento passa a ser salva no estado da última execução, permitindo redução progressiva do score entre uma tentativa e outra.
- O matching do reprocessamento agora usa o score efetivo calculado para aquela tentativa, em vez de reutilizar sempre o valor padrão.
- O reprocessamento preserva o histórico anterior de nomes estáveis já resolvidos, evitando perder associações corretas de rodadas anteriores.
- Correção na geração de placeholders verdes durante o reprocessamento para evitar criar placeholder em frases que já tinham sido atendidas em execução anterior.
- Melhoria no fluxo de reprocessamento para priorizar primeiro as cenas realmente pendentes salvas no estado anterior.
- Ajustes gerais no salvamento e carregamento de configuração para incluir os novos parâmetros de reprocessamento.
- Corrigido bug onde o nome do arquivo a ser renomeado não podia ter acentos ou caracteres especiais.
"""

# Placeholder (sem cena)
PLACEHOLDER_W = 1920
PLACEHOLDER_H = 1080
PLACEHOLDER_BG = (0, 255, 0)          # verde
PLACEHOLDER_TEXT = (255, 255, 255)    # branco
PLACEHOLDER_STROKE = (0, 0, 0)        # contorno preto

# ---------------------------
# Modelos Gemini
# ---------------------------
GEMINI_IMAGE_MODEL_CHEAP = "gemini-2.5-flash-lite"   # padrão (barato)
# fallback intermediário (se PRO indisponível)
GEMINI_IMAGE_MODEL_MID = "gemini-2.5-flash"
# fallback premium (quando disponível)
GEMINI_IMAGE_MODEL_PRO = "gemini-2.5-pro"
GEMINI_VIDEO_MODEL = "gemini-2.5-flash"              # Standard p/ vídeo
GEMINI_EMBEDDING_MODEL = "gemini-embedding-001"

# Embeddings / matching
TOP_K_PER_SCENE = 25
MIN_SIMILARITY = 0.30
MIN_ASSIGN_SCORE = 0.82

# fator multiplicativo para o score mínimo ao reprocessar pendências
REPROCESS_MIN_ASSIGN_SCORE_FACTOR = 0.95

SCENE_REUSE_SCORE_DECAY = 0.92

# Imagem enviada para descrição (economiza custo/tempo)
DESC_MAX_SIDE = 1024

VIDEO_CENTER_CLIP_SECONDS = 11.0

# Fallback (imagem): critérios
IMAGE_FALLBACK_CONFIDENCE_THRESHOLD = 0.60

DIFFICULTY_ENUM = [
    "texto_miudo",
    "muitos_elementos",
    "baixa_luz",
    "movimento_desfocado",
    "imagem_de_tela",
    "baixa_resolucao",
    "angulo_ruim",
]

DIFFICULTY_TRIGGERS_FOR_FALLBACK = {
    "texto_miudo",
    "muitos_elementos",
    "baixa_luz",
    "movimento_desfocado",
    "imagem_de_tela",
    "baixa_resolucao",
}

SCENE_DESC_JSON_SCHEMA = {
    "type": "object",
    "additionalProperties": False,
    "properties": {
        "summary": {"type": "string"},
        "keywords": {"type": "array", "items": {"type": "string"}},
        "confidence": {"type": "number", "minimum": 0, "maximum": 1},
        "doubts": {"type": "array", "items": {"type": "string"}},
        "difficulty": {"type": "array", "items": {"type": "string", "enum": DIFFICULTY_ENUM}},
    },
    "required": ["summary", "keywords", "confidence", "doubts", "difficulty"],
}


def load_config() -> dict:
    try:
        if os.path.exists(CONFIG_PATH):
            with open(CONFIG_PATH, "r", encoding="utf-8") as f:
                return json.load(f)
    except Exception:
        pass
    return {}


def save_config(data: dict) -> None:
    try:
        # ✅ cria /cache se não existir
        os.makedirs(os.path.dirname(CONFIG_PATH), exist_ok=True)
        with open(CONFIG_PATH, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception:
        pass

# ---------------------------
# Utilidades (arquivos / texto)
# ---------------------------


def is_video(path: str) -> bool:
    return os.path.splitext(path.lower())[1] in VIDEO_EXTS


def is_image(path: str) -> bool:
    return os.path.splitext(path.lower())[1] in IMAGE_EXTS


def sanitize_filename(name: str, max_len: int = 120) -> str:
    name = re.sub(f"[{re.escape(INVALID_FILENAME_CHARS)}]", " ", name)
    name = re.sub(r"\s+", " ", name).strip()

    if not name:
        name = "cena_sem_nome"

    if len(name) > max_len:
        name = name[:max_len].rstrip()

    name = name.rstrip(". ").strip()
    if not name:
        name = "cena_sem_nome"

    reserved = {"CON", "PRN", "AUX", "NUL"} | {f"COM{i}" for i in range(1, 10)} | {
        f"LPT{i}" for i in range(1, 10)}
    if name.upper() in reserved:
        name = "_" + name

    return name


def unique_path(directory: str, base_name: str, ext: str) -> str:
    candidate = os.path.join(directory, base_name + ext)
    if not os.path.exists(candidate):
        return candidate

    i = 2
    while True:
        candidate = os.path.join(directory, f"{base_name}__{i}{ext}")
        if not os.path.exists(candidate):
            return candidate
        i += 1


def first_n_words(text: str, n: int) -> str:
    words = [w.strip() for w in text.split() if w.strip()]
    words = [w.strip('“”"\'') for w in words]
    return " ".join(words[:n]).strip()


def first_n_words_filename(text: str, n: int) -> str:
    return sanitize_filename(first_n_words(text, n))


def split_sentences_by_period_only(script: str) -> List[str]:
    s = script.replace("\r", "\n")
    s = re.sub(r"\s+", " ", s).strip()
    if not s:
        return []

    parts = s.split(".")
    sentences: List[str] = []

    ends_with_period = s.endswith(".")

    for idx, part in enumerate(parts):
        part = part.strip()
        if not part:
            continue

        if idx < len(parts) - 1 or ends_with_period:
            sentences.append(part + ".")
        else:
            sentences.append(part)

    return sentences


def group_sentences(sentences: List[str], sentences_per_chunk: int) -> List[str]:
    if sentences_per_chunk < 1:
        sentences_per_chunk = 1

    chunks: List[str] = []
    for i in range(0, len(sentences), sentences_per_chunk):
        group = " ".join(sentences[i:i + sentences_per_chunk]).strip()
        if group:
            chunks.append(group)
    return chunks


def count_words(text: str) -> int:
    # conta palavras de um jeito simples (bom o suficiente p/ roteiros)
    return len([w for w in (text or "").split() if w.strip()])


def expand_script_items_by_word_limit(
    script_items: List[str],
    word_limit: int,
) -> List[str]:
    """
    Divide itens longos em sub-itens para permitir MAIS de uma cena.
    Regra:
      - word_limit = X
      - metade = ceil(X/2)
      - cria um item extra só se o resto tiver >= metade
      - se resto < metade, junta no último item (não cria um item muito curto)

    Exemplos com X=10:
      14 -> 1 item (14)
      15 -> 2 itens (10 + 5)
      24 -> 2 itens (10 + 14)
      25 -> 3 itens (10 + 10 + 5)
    """
    if word_limit < 2:
        return script_items

    min_tail = (word_limit + 1) // 2  # metade arredondando pra cima
    out: List[str] = []

    for text in script_items:
        words = [w for w in (text or "").split() if w.strip()]
        n = len(words)

        if n <= word_limit:
            out.append(text)
            continue

        k = n // word_limit        # quantos blocos cheios
        r = n % word_limit         # resto

        # se não chega nem num bloco cheio, não divide
        if k <= 0:
            out.append(text)
            continue

        # empilha os primeiros (k-1) blocos cheios
        idx = 0
        for _ in range(max(0, k - 1)):
            part = " ".join(words[idx: idx + word_limit]).strip()
            if part:
                out.append(part)
            idx += word_limit

        # agora sobram (word_limit + r) palavras (ou só word_limit se r==0)
        remaining = words[idx:]

        if r >= min_tail:
            # cria 1 bloco cheio + 1 bloco final (resto)
            part1 = " ".join(remaining[:word_limit]).strip()
            part2 = " ".join(remaining[word_limit:]).strip()
            if part1:
                out.append(part1)
            if part2:
                out.append(part2)
        else:
            # não cria item pequeno: junta tudo no último
            part = " ".join(remaining).strip()
            if part:
                out.append(part)

    return out


def build_script_items(
    roteiro_text: str,
    sentences_per_chunk: int = 1,
    split_long_phrases: bool = False,
    split_words_threshold: int = 10,
) -> List[str]:
    sentences = split_sentences_by_period_only(roteiro_text)
    items = group_sentences(sentences, sentences_per_chunk)

    if split_long_phrases:
        items = expand_script_items_by_word_limit(
            items,
            word_limit=split_words_threshold,
        )

    return [x.strip() for x in items if x and x.strip()]

# ---------------------------
# Placeholder (imagem verde) para frases sem cena
# ---------------------------


def _try_load_font(size: int):
    try:
        return ImageFont.truetype("arial.ttf", size=size)
    except Exception:
        try:
            return ImageFont.truetype("DejaVuSans.ttf", size=size)
        except Exception:
            return ImageFont.load_default()


def _wrap_text_lines(draw: ImageDraw.ImageDraw, text: str, font, max_width: int) -> List[str]:
    words = text.split()
    lines: List[str] = []
    current = ""

    for w in words:
        test = (current + " " + w).strip()
        bbox = draw.textbbox((0, 0), test, font=font)
        if (bbox[2] - bbox[0]) <= max_width:
            current = test
        else:
            if current:
                lines.append(current)
            current = w

    if current:
        lines.append(current)

    return lines


def create_green_placeholder_image(text: str, out_path: str):
    img = Image.new("RGB", (PLACEHOLDER_W, PLACEHOLDER_H), PLACEHOLDER_BG)
    draw = ImageDraw.Draw(img)

    margin = 120
    max_w = PLACEHOLDER_W - (margin * 2)
    max_h = PLACEHOLDER_H - (margin * 2)

    header = "SEM CENA"
    body = text.strip()

    font_size = 72
    header_size = 84

    while font_size >= 18:
        header_font = _try_load_font(header_size)
        body_font = _try_load_font(font_size)

        header_bbox = draw.textbbox((0, 0), header, font=header_font)
        header_h = header_bbox[3] - header_bbox[1]

        body_lines = _wrap_text_lines(draw, body, body_font, max_w)
        line_heights = []
        for ln in body_lines:
            b = draw.textbbox((0, 0), ln, font=body_font)
            line_heights.append(b[3] - b[1])

        body_h = sum(line_heights) + (len(body_lines) - 1) * 12
        total_h = header_h + 30 + body_h

        if total_h <= max_h:
            break

        font_size -= 4
        header_size = max(24, header_size - 2)

    header_font = _try_load_font(header_size)
    body_font = _try_load_font(font_size)

    header_bbox = draw.textbbox((0, 0), header, font=header_font)
    header_w = header_bbox[2] - header_bbox[0]
    header_h = header_bbox[3] - header_bbox[1]

    body_lines = _wrap_text_lines(draw, body, body_font, max_w)
    line_boxes = [draw.textbbox((0, 0), ln, font=body_font)
                  for ln in body_lines]
    line_ws = [(b[2] - b[0]) for b in line_boxes]
    line_hs = [(b[3] - b[1]) for b in line_boxes]
    body_h = sum(line_hs) + (len(body_lines) - 1) * 12

    total_h = header_h + 30 + body_h
    start_y = (PLACEHOLDER_H - total_h) // 2

    header_x = (PLACEHOLDER_W - header_w) // 2
    draw.text(
        (header_x, start_y),
        header,
        font=header_font,
        fill=PLACEHOLDER_TEXT,
        stroke_width=3,
        stroke_fill=PLACEHOLDER_STROKE,
    )

    y = start_y + header_h + 30
    for ln, w, h in zip(body_lines, line_ws, line_hs):
        x = (PLACEHOLDER_W - w) // 2
        draw.text(
            (x, y),
            ln,
            font=body_font,
            fill=PLACEHOLDER_TEXT,
            stroke_width=2,
            stroke_fill=PLACEHOLDER_STROKE,
        )
        y += h + 12

    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    img.save(out_path, format="PNG")


# ---------------------------
# Gemini: descrição estruturada (imagem/vídeo)
# ---------------------------

@dataclass
class SceneDesc:
    path: str
    desc_text: str
    confidence: float
    used_model: str
    doubts: List[str]
    difficulty: List[str]
    fallback_reason: str = ""
    content_id: str = ""     # ID do conteúdo do arquivo
    from_cache: bool = False  # veio do cache?


@dataclass
class FallbackPolicy:
    allow_pro: bool = True
    pro_disabled: bool = False
    pro_disabled_reason: str = ""
    pro_fail_count: int = 0


def _pil_to_jpeg_bytes(img: Image.Image, max_side: int = 1024, quality: int = 85) -> bytes:
    img = img.convert("RGB")
    w, h = img.size
    scale = min(max_side / max(w, h), 1.0)
    if scale < 1.0:
        img = img.resize((int(w * scale), int(h * scale)), Image.LANCZOS)

    buf = io.BytesIO()
    img.save(buf, format="JPEG", quality=quality, optimize=True)
    return buf.getvalue()


def _prepare_image_bytes_for_gemini(path: str, max_side: int = 1024) -> Tuple[bytes, str]:
    img = Image.open(path)
    jpeg_bytes = _pil_to_jpeg_bytes(img, max_side=max_side, quality=85)
    return jpeg_bytes, "image/jpeg"


def _safe_json_loads(s: str) -> dict:
    s = (s or "").strip()
    if not s:
        raise ValueError("Resposta vazia.")
    try:
        return json.loads(s)
    except Exception:
        i = s.find("{")
        j = s.rfind("}")
        if i != -1 and j != -1 and j > i:
            return json.loads(s[i:j+1])
        raise


def _compute_content_id(path: str, chunk_size: int = 1024 * 1024) -> str:
    """
    ID do conteúdo do arquivo (rápido e bem confiável):
    - usa tamanho + 1MB do começo + 1MB do fim
    - para arquivos pequenos, usa tudo
    """
    st = os.stat(path)
    size = st.st_size

    h = hashlib.sha256()
    h.update(str(size).encode("utf-8"))
    h.update(b"|")

    with open(path, "rb") as f:
        first = f.read(min(chunk_size, size))
        h.update(first)

        if size > chunk_size:
            # lê o final
            read_tail = min(chunk_size, size)
            f.seek(max(0, size - read_tail))
            tail = f.read(read_tail)
            h.update(tail)

    return h.hexdigest()


def load_scene_cache() -> dict:
    """
    Estrutura:
    {
      "version": 1,
      "items": {
         "<content_id>": {
            "desc_text": "...",
            "confidence": 0.9,
            "used_model": "...",
            "doubts": [...],
            "difficulty": [...],
            "fallback_reason": "...",
            "created_at": "...",
            "last_seen_at": "...",
            "last_seen_path": "...",
            "seen_count": 3
         }
      }
    }
    """
    try:
        if os.path.exists(SCENE_CACHE_PATH):
            with open(SCENE_CACHE_PATH, "r", encoding="utf-8") as f:
                data = json.load(f)
                if isinstance(data, dict) and data.get("version") == SCENE_CACHE_VERSION:
                    if "items" not in data or not isinstance(data["items"], dict):
                        data["items"] = {}
                    return data
    except Exception:
        pass

    return {"version": SCENE_CACHE_VERSION, "items": {}}


def save_scene_cache(cache: dict) -> None:
    try:
        # ✅ cria /cache se não existir
        os.makedirs(os.path.dirname(SCENE_CACHE_PATH), exist_ok=True)

        tmp = SCENE_CACHE_PATH + ".tmp"
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump(cache, f, ensure_ascii=False, indent=2)
        os.replace(tmp, SCENE_CACHE_PATH)  # troca atômica
    except Exception:
        pass


def _build_scene_cache_key(
    path: str,
    content_id: str,
    include_video_audio: bool,
    clip_duration_s: Optional[float] = None,
) -> str:
    """
    Monta uma chave de cache mais inteligente.

    Para vídeo, a chave diferencia:
    - conteúdo do arquivo
    - duração do recorte usado
    - se o envio foi com áudio ou sem áudio

    Para imagem, a chave diferencia:
    - conteúdo do arquivo
    - tamanho máximo usado no preparo da imagem
    """
    if is_video(path):
        if clip_duration_s is not None:
            clip_tag = f"center@{clip_duration_s:.3f}"
        else:
            clip_tag = "full"

        audio_tag = "aud1" if include_video_audio else "aud0"
        return f"vid|{content_id}|{clip_tag}|{audio_tag}"

    return f"img|{content_id}|side={DESC_MAX_SIDE}"


def _find_compatible_scene_cache_entry(
    items: dict,
    path: str,
    content_id: str,
    include_video_audio: bool,
    clip_duration_s: Optional[float] = None,
):
    """
    Procura cache compatível nesta ordem:

    1) formato novo ideal (chave rica)
    2) formato simples legado (somente para imagem)
    3) formato antigo do cache antigo: hash|...|aud0 / aud1

    Para vídeo, só reaproveita o cache antigo se o modo de áudio combinar.
    """
    # 1) formato novo ideal
    preferred_key = _build_scene_cache_key(
        path=path,
        content_id=content_id,
        include_video_audio=include_video_audio,
        clip_duration_s=clip_duration_s,
    )
    direct = items.get(preferred_key)
    if isinstance(direct, dict):
        return preferred_key, direct

    # 2) legado do "cache simples" atual
    # Para imagem, pode reaproveitar sem medo.
    if not is_video(path):
        simple = items.get(content_id)
        if isinstance(simple, dict):
            return content_id, simple

    # 3) formato antigo: hash|modo|audX
    if is_video(path):
        preferred_audio_suffix = "|aud1" if include_video_audio else "|aud0"
        legacy_prefix = content_id + "|"

        for key, value in items.items():
            if not isinstance(value, dict):
                continue
            if key.startswith(legacy_prefix) and key.endswith(preferred_audio_suffix):
                return key, value

    return None, None


def _scene_desc_to_cache_entry(desc: SceneDesc) -> dict:
    now = datetime.utcnow().isoformat() + "Z"
    return {
        "desc_text": desc.desc_text,
        "confidence": float(desc.confidence),
        "used_model": desc.used_model,
        "doubts": list(desc.doubts) if desc.doubts else [],
        "difficulty": list(desc.difficulty) if desc.difficulty else [],
        "fallback_reason": desc.fallback_reason or "",
        "created_at": now,
        "last_seen_at": now,
        "last_seen_path": desc.path,
        "seen_count": 1,
    }


def _scene_desc_from_cache_entry(path: str, content_id: str, entry: dict) -> SceneDesc:
    # Atualiza metadados de "última vez visto" (fazemos isso no worker)
    return SceneDesc(
        path=path,
        desc_text=str(entry.get("desc_text", "")).strip(),
        confidence=float(entry.get("confidence", 0.0) or 0.0),
        used_model=str(entry.get("used_model", "")).strip(),
        doubts=[str(x).strip()
                for x in (entry.get("doubts") or []) if str(x).strip()],
        difficulty=[str(x).strip() for x in (
            entry.get("difficulty") or []) if str(x).strip()],
        fallback_reason=str(entry.get("fallback_reason", "") or "").strip(),
        content_id=content_id,
        from_cache=True,
    )


def _repair_to_schema(
    client: genai.Client,
    model_for_repair: str,
    broken_text: str,
    retries: int = 2,
    max_output_tokens: int = 260,
) -> dict:
    """
    Repara uma resposta que veio como "JSON quebrado" (ou nem JSON era),
    pedindo ao modelo para reescrever SOMENTE um JSON válido no schema.

    Usa um modelo barato (ex.: flash-lite) porque aqui é só texto.
    """
    last_err = None
    prompt = (
        "O texto abaixo deveria ser um JSON, mas veio inválido.\n"
        "Reescreva SOMENTE um JSON válido seguindo EXATAMENTE este schema.\n"
        "Não inclua markdown, não inclua explicações, não inclua texto fora do JSON.\n\n"
        "TEXTO PARA REPARAR:\n"
        f"{broken_text}"
    )

    for attempt in range(retries):
        try:
            resp = client.models.generate_content(
                model=model_for_repair,
                contents=[prompt],
                config={
                    "temperature": 0,
                    "max_output_tokens": max_output_tokens,
                    "response_mime_type": "application/json",
                    "response_json_schema": SCENE_DESC_JSON_SCHEMA,
                },
            )

            parsed = getattr(resp, "parsed", None)
            if parsed is not None:
                if hasattr(parsed, "model_dump"):
                    out = parsed.model_dump()
                    if isinstance(out, dict):
                        return out
                if isinstance(parsed, dict):
                    return parsed

            return _safe_json_loads(getattr(resp, "text", "") or "")

        except Exception as e:
            last_err = e
            time.sleep(0.4 * (attempt + 1))

    raise RuntimeError(f"Falha ao reparar JSON: {last_err}")


def _extract_retry_seconds_from_message(msg: str) -> Optional[float]:
    """
    Alguns erros vêm com: 'Please retry in 37.857191072s.'
    """
    if not msg:
        return None
    m = re.search(r"Please retry in\s+([0-9]+(?:\.[0-9]+)?)s", msg)
    if m:
        try:
            return float(m.group(1))
        except Exception:
            return None
    # Às vezes vem em ms: 'Please retry in 862.571829ms.'
    m2 = re.search(r"Please retry in\s+([0-9]+(?:\.[0-9]+)?)ms", msg)
    if m2:
        try:
            return float(m2.group(1)) / 1000.0
        except Exception:
            return None
    return None


def _is_quota_pro_zero_error(err: Exception) -> bool:
    """
    Detecta o padrão do seu log:
    - 429 RESOURCE_EXHAUSTED
    - menciona free_tier e model gemini-2.5-pro
    - ou 'limit: 0'
    """
    msg = str(err)
    msg_low = msg.lower()
    if "429" in msg_low and "resource_exhausted" in msg_low:
        if "gemini-2.5-pro" in msg_low and ("limit: 0" in msg_low or "free_tier" in msg_low):
            return True
    return False


def _gemini_generate_structured(
    client: genai.Client,
    model: str,
    contents,
    retries: int = 3,
    max_output_tokens: int = 280,
    base_sleep_s: float = 0.25,
) -> dict:
    last_err = None

    for attempt in range(retries):
        try:
            resp = client.models.generate_content(
                model=model,
                contents=contents,
                config={
                    "temperature": 0,
                    "max_output_tokens": max_output_tokens,
                    "response_mime_type": "application/json",
                    "response_json_schema": SCENE_DESC_JSON_SCHEMA,
                },
            )

            # 1) Preferir "parsed" (quando o SDK conseguiu validar/parsing)
            parsed = getattr(resp, "parsed", None)
            if parsed is not None:
                if hasattr(parsed, "model_dump"):
                    out = parsed.model_dump()
                    if isinstance(out, dict):
                        return out
                if isinstance(parsed, dict):
                    return parsed

            # 2) Senão, tenta ler o texto como JSON
            raw = getattr(resp, "text", "") or ""
            try:
                return _safe_json_loads(raw)
            except Exception:
                # 3) Se o texto veio quebrado, faz "reparo" automático (barato)
                return _repair_to_schema(
                    client=client,
                    # flash-lite (barato)
                    model_for_repair=GEMINI_IMAGE_MODEL_CHEAP,
                    broken_text=raw,
                    retries=2,
                    max_output_tokens=260,
                )

        except Exception as e:
            last_err = e

            retry_s = _extract_retry_seconds_from_message(str(e))
            if retry_s is not None:
                time.sleep(max(0.0, retry_s))
            else:
                time.sleep(base_sleep_s * (attempt + 1))

    raise RuntimeError(
        f"Falha ao gerar descrição no Gemini ({model}): {last_err}")


def _should_fallback_image(data: dict) -> Tuple[bool, str]:
    conf = float(data.get("confidence", 0.0) or 0.0)
    doubts = [str(x).strip()
              for x in (data.get("doubts") or []) if str(x).strip()]
    diff = [str(x).strip()
            for x in (data.get("difficulty") or []) if str(x).strip()]

    if conf < IMAGE_FALLBACK_CONFIDENCE_THRESHOLD:
        return True, f"confidence baixa ({conf:.2f} < {IMAGE_FALLBACK_CONFIDENCE_THRESHOLD:.2f})"
    if any(d in DIFFICULTY_TRIGGERS_FOR_FALLBACK for d in diff) and conf < 0.80:
        return True, f"dificuldade marcada ({', '.join(diff)})"
    if len(doubts) > 0:
        return True, f"dúvidas retornadas ({len(doubts)} item(ns))"
    return False, ""


def _build_scene_desc_from_data(path: str, data: dict, used_model: str, fallback_reason: str = "") -> SceneDesc:
    summary = (data.get("summary") or "").strip()
    keywords = [str(k).strip()
                for k in (data.get("keywords") or []) if str(k).strip()]
    conf = float(data.get("confidence", 0.0) or 0.0)
    doubts = [str(x).strip()
              for x in (data.get("doubts") or []) if str(x).strip()]
    difficulty = [str(x).strip()
                  for x in (data.get("difficulty") or []) if str(x).strip()]

    desc_text = summary
    if keywords:
        desc_text += " Palavras-chave: " + ", ".join(keywords)

    return SceneDesc(
        path=path,
        desc_text=desc_text,
        confidence=conf,
        used_model=used_model,
        doubts=doubts,
        difficulty=difficulty,
        fallback_reason=fallback_reason,
    )


def describe_image_with_fallback(
    client: genai.Client,
    image_path: str,
    policy: FallbackPolicy,
    log_fn=None,
) -> SceneDesc:
    img_bytes, mime = _prepare_image_bytes_for_gemini(
        image_path, max_side=DESC_MAX_SIDE)

    prompt = (
        "Você é um assistente que descreve CENAS VISUAIS em português.\n"
        "Analise a imagem e retorne SOMENTE um JSON seguindo o schema.\n"
        "Regras:\n"
        "- summary: 1 a 2 frases curtas e objetivas.\n"
        "- keywords: 5 a 12 palavras-chave curtas.\n"
        "- confidence: 0 a 1.\n"
        "- doubts: liste rapidamente o que pode estar incerto (se nada, lista vazia).\n"
        "- difficulty: marque dificuldades percebidas (ex.: texto miúdo, muita coisa acontecendo, baixa luz...).\n"
    )

    contents = [
        prompt,
        types.Part.from_bytes(data=img_bytes, mime_type=mime),
    ]

    # Passo 1 (barato) — se isso der certo, a gente nunca perde.
    cheap_data = _gemini_generate_structured(
        client, GEMINI_IMAGE_MODEL_CHEAP, contents, max_output_tokens=280)
    need_fallback, reason = _should_fallback_image(cheap_data)
    cheap_desc = _build_scene_desc_from_data(
        image_path, cheap_data, GEMINI_IMAGE_MODEL_CHEAP)

    if not need_fallback:
        return cheap_desc

    if log_fn:
        log_fn(f"Fallback sugerido para esta imagem: {reason}")

    # Passo 2 (premium) — PRO, mas só se:
    # - usuário permitiu
    # - e ainda não foi desabilitado por quota
    if policy.allow_pro and (not policy.pro_disabled):
        try:
            pro_data = _gemini_generate_structured(
                client, GEMINI_IMAGE_MODEL_PRO, contents, max_output_tokens=320)
            return _build_scene_desc_from_data(image_path, pro_data, GEMINI_IMAGE_MODEL_PRO, fallback_reason=reason)
        except Exception as e:
            policy.pro_fail_count += 1
            if _is_quota_pro_zero_error(e):
                policy.pro_disabled = True
                policy.pro_disabled_reason = "Quota do gemini-2.5-pro no free tier está 0 (429)."
                if log_fn:
                    log_fn(
                        "PRO indisponível (quota=0). Vou desativar o PRO e usar fallback alternativo / barato.")
            else:
                if log_fn:
                    log_fn(
                        f"Fallback PRO falhou. Vou tentar fallback alternativo / barato. Motivo: {e}")

            if policy.pro_fail_count >= 3 and not policy.pro_disabled:
                policy.pro_disabled = True
                policy.pro_disabled_reason = f"PRO falhou {policy.pro_fail_count}x (resposta vazia/JSON inválido)."
                if log_fn:
                    log_fn(
                        f"Desativando PRO nesta execução: {policy.pro_disabled_reason}")

    # Fallback alternativo (melhor que lite): gemini-2.5-flash
    try:
        mid_data = _gemini_generate_structured(
            client, GEMINI_IMAGE_MODEL_MID, contents, max_output_tokens=320)
        return _build_scene_desc_from_data(image_path, mid_data, GEMINI_IMAGE_MODEL_MID, fallback_reason=reason)
    except Exception as e:
        if log_fn:
            log_fn(
                f"Fallback alternativo (flash) falhou. Vou manter o resultado do flash-lite. Motivo: {e}")
        return cheap_desc


def _prepare_safe_video_upload_path(src_path: str) -> Tuple[str, bool]:
    """
    Garante um caminho ASCII simples para upload do vídeo.

    Isso evita erro de encoding em alguns ambientes/SDKs quando o nome do
    arquivo tem caracteres como ã, é, ç etc.

    Retorna:
      - caminho que deve ser enviado
      - True se criou cópia temporária e precisa apagar no final
    """
    try:
        src_path.encode("ascii")
        return src_path, False
    except UnicodeEncodeError:
        pass

    ext = os.path.splitext(src_path)[1] or ".mp4"
    tmp = tempfile.NamedTemporaryFile(
        delete=False,
        prefix="video_upload_safe_",
        suffix=ext,
    )
    tmp_path = tmp.name
    tmp.close()

    shutil.copy2(src_path, tmp_path)
    return tmp_path, True


def _wait_file_active(client: genai.Client, uploaded_file, timeout_s: int = 600) -> None:
    t0 = time.time()
    f = uploaded_file
    while getattr(f, "state", None) is not None and getattr(f.state, "name", "") == "PROCESSING":
        if time.time() - t0 > timeout_s:
            raise RuntimeError(
                "Timeout esperando o vídeo ficar ACTIVE no File API.")
        time.sleep(2)
        f = client.files.get(name=f.name)

    if getattr(f, "state", None) is not None and getattr(f.state, "name", "") == "FAILED":
        raise RuntimeError("Upload do vídeo falhou (state=FAILED).")


def describe_video(
    client: genai.Client,
    video_path: str,
    delete_after: bool = True,
    clip_start_s: Optional[float] = None,
    clip_duration_s: Optional[float] = None,
    include_audio: bool = True,
    log_fn=None,
) -> SceneDesc:
    prompt = (
        "Você é um assistente que descreve VÍDEOS em português.\n"
        + ("Analise o vídeo (visual + áudio) e retorne SOMENTE um JSON seguindo o schema.\n"
           if include_audio else
           "Analise o vídeo APENAS pelo visual (sem áudio) e retorne SOMENTE um JSON seguindo o schema.\n")
        + "Regras:\n"
        "- summary: 1 a 2 frases curtas descrevendo o que mais importa no vídeo.\n"
        "- keywords: 5 a 12 palavras-chave.\n"
        "- confidence: 0 a 1.\n"
        "- doubts: liste rapidamente o que pode estar incerto (se nada, lista vazia).\n"
        "- difficulty: marque dificuldades percebidas.\n"
    )

    temp_clip_path = None
    temp_upload_copy_path = None
    upload_path = video_path
    uploaded = None

    if clip_start_s is not None and clip_duration_s is not None:
        temp_clip_path, ffmpeg_error = _ffmpeg_extract_clip_to_temp(
            video_path,
            clip_start_s,
            clip_duration_s,
            include_audio=include_audio
        )
        if temp_clip_path:
            upload_path = temp_clip_path
            if log_fn:
                self_audio = "com áudio" if include_audio else "sem áudio"
                log_fn(f"Usando trecho recortado ({self_audio}).")
        else:
            if log_fn:
                log_fn(
                    "[WARN] Não consegui recortar o vídeo. Vou enviar o vídeo inteiro.")
                if ffmpeg_error:
                    log_fn(f"[FFMPEG] {ffmpeg_error}")
    else:
        if not include_audio:
            temp_clip_path, ffmpeg_error = _ffmpeg_remove_audio_to_temp(
                video_path)
            if temp_clip_path:
                upload_path = temp_clip_path
                if log_fn:
                    log_fn("Enviando vídeo SEM áudio (áudio removido via ffmpeg).")
            else:
                if log_fn:
                    log_fn(
                        "[WARN] Não consegui remover o áudio. Vou enviar com áudio mesmo.")
                    if ffmpeg_error:
                        log_fn(f"[FFMPEG] {ffmpeg_error}")

    upload_path, created_ascii_copy = _prepare_safe_video_upload_path(
        upload_path)
    if created_ascii_copy:
        temp_upload_copy_path = upload_path
        if log_fn:
            log_fn(
                "[UPLOAD] Nome/caminho com acento detectado. "
                "Criei uma cópia temporária com nome simples para evitar erro de encoding."
            )

    mime_type = mimetypes.guess_type(video_path)[0] or "video/mp4"

    try:
        with open(upload_path, "rb") as f:
            uploaded = client.files.upload(
                file=f,
                config={"mime_type": mime_type},
            )

        _wait_file_active(client, uploaded)

        contents = [prompt, uploaded]
        data = _gemini_generate_structured(
            client, GEMINI_VIDEO_MODEL, contents,
            retries=5,
            max_output_tokens=520
        )
        return _build_scene_desc_from_data(video_path, data, GEMINI_VIDEO_MODEL)
    finally:
        if delete_after and uploaded is not None:
            try:
                client.files.delete(name=uploaded.name)
            except Exception:
                pass
        if temp_clip_path:
            try:
                os.remove(temp_clip_path)
            except Exception:
                pass
        if temp_upload_copy_path:
            try:
                os.remove(temp_upload_copy_path)
            except Exception:
                pass


def describe_single_scene(
    path: str,
    client: genai.Client,
    log_fn=None,
    allow_pro_fallback: bool = True,
    include_video_audio: bool = True,
    stop_event=None,
    scene_cache: Optional[dict] = None,
) -> SceneDesc:
    if stop_event is not None and stop_event.is_set():
        raise ProcessingCancelled("Processamento cancelado pelo usuário.")

    own_cache = scene_cache is None
    cache = scene_cache if scene_cache is not None else load_scene_cache()
    items = cache.setdefault("items", {})

    content_id = _compute_content_id(path)

    # Calcula o perfil da análise do vídeo ANTES de consultar o cache,
    # para que a chave do cache reflita o contexto real.
    clip_start_s = None
    clip_duration_s = None

    if is_video(path):
        duration_s = _ffprobe_duration_seconds(path)
        if duration_s is not None:
            clip_start_s, clip_duration_s = _compute_center_clip_window(
                duration_s,
                VIDEO_CENTER_CLIP_SECONDS,
            )

    cache_key_used, cached_entry = _find_compatible_scene_cache_entry(
        items=items,
        path=path,
        content_id=content_id,
        include_video_audio=include_video_audio,
        clip_duration_s=clip_duration_s,
    )

    if log_fn:
        preferred_cache_key = _build_scene_cache_key(
            path=path,
            content_id=content_id,
            include_video_audio=include_video_audio,
            clip_duration_s=clip_duration_s,
        )
        log_fn(f"[CACHE DEBUG] SCENE_CACHE_PATH = {SCENE_CACHE_PATH}")
        log_fn(f"[CACHE DEBUG] itens no cache = {len(items)}")
        log_fn(f"[CACHE DEBUG] content_id = {content_id}")
        log_fn(f"[CACHE DEBUG] preferred_cache_key = {preferred_cache_key}")
        log_fn(
            f"[CACHE DEBUG] cache_hit = {'SIM' if isinstance(cached_entry, dict) else 'NAO'}"
        )

    if isinstance(cached_entry, dict):
        now = datetime.utcnow().isoformat() + "Z"
        cached_entry["last_seen_at"] = now
        cached_entry["last_seen_path"] = path
        cached_entry["seen_count"] = int(
            cached_entry.get("seen_count", 0) or 0
        ) + 1

        if own_cache:
            save_scene_cache(cache)

        desc = _scene_desc_from_cache_entry(path, content_id, cached_entry)
        if log_fn:
            log_fn(
                f"[CACHE] Reaproveitado: {os.path.basename(path)} | "
                f"cache_key={cache_key_used} | "
                f"modelo={desc.used_model} | conf={desc.confidence:.2f}"
            )
        return desc

    if log_fn:
        tipo = "vídeo" if is_video(path) else "imagem"
        log_fn(f"[ANALISANDO] {os.path.basename(path)} ({tipo})")

    if is_video(path):
        desc = describe_video(
            client=client,
            video_path=path,
            delete_after=True,
            clip_start_s=clip_start_s,
            clip_duration_s=clip_duration_s,
            include_audio=include_video_audio,
            log_fn=log_fn,
        )
    else:
        policy = FallbackPolicy(allow_pro=allow_pro_fallback)
        desc = describe_image_with_fallback(
            client=client,
            image_path=path,
            policy=policy,
            log_fn=log_fn,
        )

    desc.path = path
    desc.content_id = content_id
    desc.from_cache = False

    cache_key_to_save = _build_scene_cache_key(
        path=path,
        content_id=content_id,
        include_video_audio=include_video_audio,
        clip_duration_s=clip_duration_s,
    )

    items[cache_key_to_save] = _scene_desc_to_cache_entry(desc)
    items[cache_key_to_save]["last_seen_path"] = path

    if log_fn:
        log_fn(
            f"[CACHE DEBUG] cache salvo para cache_key = {cache_key_to_save}")

    if own_cache:
        save_scene_cache(cache)

    if log_fn:
        extra = f" | fallback={desc.fallback_reason}" if desc.fallback_reason else ""
        log_fn(
            f"[OK] {os.path.basename(path)} | "
            f"modelo={desc.used_model} | conf={desc.confidence:.2f}{extra}"
        )

    return desc


_DESCRIBE_MAX_WORKERS = 4  # respeita rate-limit da API Gemini


def describe_all_scenes(
    selected_files: List[str],
    client: genai.Client,
    log_fn,
    progress_fn=None,
    allow_pro_fallback: bool = True,
    include_video_audio: bool = True,
    stop_event=None,
) -> List[SceneDesc]:
    cache = load_scene_cache()
    total = len(selected_files)
    results: List[Optional[SceneDesc]] = [None] * total

    progress_lock = threading.Lock()
    completed_count = [0]

    def _process_one(idx: int, file_path: str):
        if stop_event is not None and stop_event.is_set():
            raise ProcessingCancelled("Processamento cancelado pelo usuário.")

        if log_fn:
            log_fn("-" * 60)
            log_fn(f"[{idx}/{total}] {os.path.basename(file_path)}")

        desc = describe_single_scene(
            path=file_path,
            client=client,
            log_fn=log_fn,
            allow_pro_fallback=allow_pro_fallback,
            include_video_audio=include_video_audio,
            stop_event=stop_event,
            scene_cache=cache,
        )

        with progress_lock:
            completed_count[0] += 1
            if progress_fn:
                progress_fn(completed_count[0])

        return idx - 1, desc  # posição 0-indexada

    with ThreadPoolExecutor(max_workers=_DESCRIBE_MAX_WORKERS) as executor:
        futures = {
            executor.submit(_process_one, idx, file_path): idx
            for idx, file_path in enumerate(selected_files, start=1)
        }

        for future in as_completed(futures):
            pos, desc = future.result()  # propaga ProcessingCancelled e outros erros
            results[pos] = desc

    save_scene_cache(cache)
    return [r for r in results if r is not None]

# ---------------------------
# Gemini: Embeddings (texto -> vetor)
# ---------------------------


def get_embeddings_batched(client: genai.Client, model: str, texts: List[str], batch_size: int = 200) -> List[List[float]]:
    batch_size = min(int(batch_size), 100)  # limite da API
    vectors: List[List[float]] = []
    for i in range(0, len(texts), batch_size):
        chunk = texts[i:i + batch_size]
        resp = client.models.embed_content(
            model=model,
            contents=chunk,
            config=types.EmbedContentConfig(task_type="SEMANTIC_SIMILARITY"),
        )
        for emb in resp.embeddings:
            vectors.append(list(emb.values))
    return vectors


def normalize_rows(mat: np.ndarray) -> np.ndarray:
    norms = np.linalg.norm(mat, axis=1, keepdims=True)
    norms[norms == 0] = 1.0
    return mat / norms


# ---------------------------
# Matching global (cena <-> frase)
# ---------------------------

@dataclass
class Assignment:
    phrase_idx: int
    scene_idx: int
    score: float


def _normalize_text_for_stable_match(text: str) -> str:
    text = (text or "").replace("\r", "\n").strip().lower()
    text = re.sub(r"\s+", " ", text)
    return text


def _phrase_key(text: str) -> str:
    return _normalize_text_for_stable_match(text)


def load_stable_match_cache() -> dict:
    try:
        if os.path.exists(STABLE_MATCH_CACHE_PATH):
            with open(STABLE_MATCH_CACHE_PATH, "r", encoding="utf-8") as f:
                data = json.load(f)
                if isinstance(data, dict) and data.get("version") == STABLE_MATCH_CACHE_VERSION:
                    if "items" not in data or not isinstance(data["items"], dict):
                        data["items"] = {}
                    return data
    except Exception:
        pass

    return {"version": STABLE_MATCH_CACHE_VERSION, "items": {}}


def save_stable_match_cache(cache: dict) -> None:
    try:
        os.makedirs(os.path.dirname(STABLE_MATCH_CACHE_PATH), exist_ok=True)

        tmp = STABLE_MATCH_CACHE_PATH + ".tmp"
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump(cache, f, ensure_ascii=False, indent=2)
        os.replace(tmp, STABLE_MATCH_CACHE_PATH)
    except Exception:
        pass


def recover_stable_assignments(
    script_items: List[str],
    scene_descs: List[SceneDesc],
    stable_cache: dict,
    log_fn,
) -> Tuple[List[Assignment], Set[int], Set[int]]:
    items = stable_cache.get("items", {})
    if not isinstance(items, dict) or not items:
        return [], set(), set()

    phrase_positions = {}
    for pi, phrase in enumerate(script_items):
        key = _phrase_key(phrase)
        phrase_positions.setdefault(key, []).append(pi)

    assignments: List[Assignment] = []
    used_phrases: Set[int] = set()
    used_scenes: Set[int] = set()

    for si, scene in enumerate(scene_descs):
        if not scene.content_id:
            continue

        entry = items.get(scene.content_id)
        if not isinstance(entry, dict):
            continue

        phrase_key = str(entry.get("phrase_key", "") or "").strip()
        if not phrase_key:
            continue

        candidate_phrase_idxs = phrase_positions.get(phrase_key, [])
        phrase_idx = next(
            (pi for pi in candidate_phrase_idxs if pi not in used_phrases),
            None,
        )
        if phrase_idx is None:
            continue

        score = float(entry.get("last_score", 1.0) or 1.0)

        assignments.append(
            Assignment(
                phrase_idx=phrase_idx,
                scene_idx=si,
                score=score,
            )
        )
        used_phrases.add(phrase_idx)
        used_scenes.add(si)

        log_fn(
            f"[ESTÁVEL] Reaproveitado match antigo: "
            f"{os.path.basename(scene.path)} -> frase #{phrase_idx + 1}"
        )

    assignments.sort(key=lambda a: a.phrase_idx)
    return assignments, used_phrases, used_scenes


def update_stable_match_cache(
    stable_cache: dict,
    assignments: List[Assignment],
    script_items: List[str],
    scene_descs: List[SceneDesc],
    n_words_for_filename: int,
) -> None:
    items = stable_cache.setdefault("items", {})
    now = datetime.utcnow().isoformat() + "Z"

    best_assignment_by_content_id = {}

    for a in assignments:
        scene = scene_descs[a.scene_idx]
        if not scene.content_id:
            continue

        prev = best_assignment_by_content_id.get(scene.content_id)
        if (prev is None) or (a.score > prev.score):
            best_assignment_by_content_id[scene.content_id] = a

    for content_id, a in best_assignment_by_content_id.items():
        scene = scene_descs[a.scene_idx]
        phrase = script_items[a.phrase_idx]

        items[scene.content_id] = {
            "content_id": scene.content_id,
            "phrase_key": _phrase_key(phrase),
            "phrase_preview": phrase[:500],
            "target_base_name": sanitize_filename(
                first_n_words(phrase, n_words_for_filename),
                max_len=140,
            ),
            "last_score": float(a.score),
            "used_model": scene.used_model,
            "updated_at": now,
            "scene_path": scene.path,
        }


def resolve_stable_output_base_name(
    content_id: str,
    phrase: str,
    stable_cache: dict,
    n_words_for_filename: int,
) -> str:
    items = stable_cache.get("items", {})
    entry = items.get(content_id, {}) if isinstance(items, dict) else {}

    current_phrase_key = _phrase_key(phrase)
    saved_phrase_key = str(entry.get("phrase_key", "") or "").strip()
    saved = str(entry.get("target_base_name", "") or "").strip()

    # Só reutiliza o nome estável salvo se a frase atual for exatamente
    # a mesma frase principal registrada no histórico estável.
    if saved and saved_phrase_key and saved_phrase_key == current_phrase_key:
        return sanitize_filename(saved, max_len=140)

    # Se esta cena estiver sendo usada em outra frase diferente,
    # gera o nome com base na frase atual para não colapsar tudo
    # no mesmo nome de arquivo.
    return sanitize_filename(
        first_n_words(phrase, n_words_for_filename),
        max_len=140,
    )


def load_last_run_state() -> dict:
    try:
        if os.path.exists(LAST_RUN_STATE_PATH):
            with open(LAST_RUN_STATE_PATH, "r", encoding="utf-8") as f:
                data = json.load(f)
                if isinstance(data, dict) and data.get("version") == LAST_RUN_STATE_VERSION:
                    return data
    except Exception:
        pass

    return {
        "version": LAST_RUN_STATE_VERSION,
        "use_script": False,
        "roteiro_text": "",
        "all_scene_paths": [],
        "pending_scene_content_ids": [],
        "pending_scene_paths": [],
        "pending_phrase_indices": [],
        "reprocess_pending_attempts": 0,
    }


def save_last_run_state(data: dict) -> None:
    try:
        os.makedirs(os.path.dirname(LAST_RUN_STATE_PATH), exist_ok=True)

        payload = dict(data)
        payload["version"] = LAST_RUN_STATE_VERSION

        tmp = LAST_RUN_STATE_PATH + ".tmp"
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump(payload, f, ensure_ascii=False, indent=2)

        os.replace(tmp, LAST_RUN_STATE_PATH)
    except Exception:
        pass


def load_undo_last_run() -> dict:
    try:
        if os.path.exists(UNDO_LAST_RUN_PATH):
            with open(UNDO_LAST_RUN_PATH, "r", encoding="utf-8") as f:
                data = json.load(f)
                if isinstance(data, dict) and data.get("version") == UNDO_LAST_RUN_VERSION:
                    if "operations" not in data or not isinstance(data["operations"], list):
                        data["operations"] = []
                    return data
    except Exception:
        pass

    return {
        "version": UNDO_LAST_RUN_VERSION,
        "created_at": "",
        "status": "empty",
        "operations": [],
    }


def save_undo_last_run(data: dict) -> None:
    try:
        os.makedirs(os.path.dirname(UNDO_LAST_RUN_PATH), exist_ok=True)

        payload = dict(data)
        payload["version"] = UNDO_LAST_RUN_VERSION

        tmp = UNDO_LAST_RUN_PATH + ".tmp"
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump(payload, f, ensure_ascii=False, indent=2)

        os.replace(tmp, UNDO_LAST_RUN_PATH)
    except Exception:
        pass


def clear_undo_last_run() -> None:
    save_undo_last_run({
        "created_at": datetime.utcnow().isoformat() + "Z",
        "status": "empty",
        "operations": [],
    })


def clear_stable_match_cache_file() -> None:
    """
    Remove o cache estável de vínculo entre frase e cena.
    Mantém intactos:
    - config
    - cache de descrição de cenas
    """
    try:
        if os.path.exists(STABLE_MATCH_CACHE_PATH):
            os.remove(STABLE_MATCH_CACHE_PATH)
    except Exception:
        pass


def build_global_assignment(
    script_items: List[str],
    scene_descs: List[SceneDesc],
    client: genai.Client,
    log_fn,
    max_uses_per_scene: int = 1,
    initial_scene_use_counts: Optional[dict] = None,
    min_similarity: Optional[float] = None,
    min_assign_score: Optional[float] = None,
) -> Tuple[List[Assignment], Set[int], Set[int]]:
    """
    Faz casamento global exato entre frases e cenas.

    O que muda em relação à versão antiga:
    - não usa mais escolha gulosa (greedy)
    - calcula a matriz completa frase x cena
    - permite repetir cena criando "slots" de uso
    - permite deixar frase sem cena quando nenhuma opção passa do limiar final
    - tenta primeiro um resolvedor exato rápido (SciPy)
    - se SciPy não existir, cai para um resolvedor exato em Python puro
    """
    if not script_items or not scene_descs:
        return [], set(), set()

    max_uses_per_scene = max(1, int(max_uses_per_scene))
    initial_scene_use_counts = {
        int(k): max(0, int(v))
        for k, v in dict(initial_scene_use_counts or {}).items()
    }

    effective_min_similarity = float(
        MIN_SIMILARITY if min_similarity is None else min_similarity
    )
    effective_min_assign_score = float(
        MIN_ASSIGN_SCORE if min_assign_score is None else min_assign_score
    )

    # Escala inteira para o otimizador trabalhar com custo estável.
    # Ex.: score 0.75321 -> 75321
    SCORE_SCALE = 100000

    log_fn(f"Gerando embeddings do roteiro ({len(script_items)} itens)...")
    script_vecs = get_embeddings_batched(
        client, GEMINI_EMBEDDING_MODEL, script_items
    )
    script_mat = normalize_rows(np.array(script_vecs, dtype=np.float32))

    log_fn(f"Gerando embeddings das cenas ({len(scene_descs)} descrições)...")
    scene_texts = [s.desc_text for s in scene_descs]
    scene_vecs = get_embeddings_batched(
        client, GEMINI_EMBEDDING_MODEL, scene_texts
    )
    scene_mat = normalize_rows(np.array(scene_vecs, dtype=np.float32))

    log_fn("Calculando matriz completa de similaridade frase x cena...")
    base_score_matrix = script_mat @ scene_mat.T

    # Ajuste pela confiança da descrição da cena.
    scene_confidence_factors = np.array(
        [(0.6 + 0.4 * float(s.confidence)) for s in scene_descs],
        dtype=np.float32,
    )
    base_score_matrix = base_score_matrix * \
        scene_confidence_factors[np.newaxis, :]

    slot_scene_indices: List[int] = []
    slot_use_numbers: List[int] = []

    for scene_idx in range(len(scene_descs)):
        already_used = initial_scene_use_counts.get(scene_idx, 0)
        if already_used >= max_uses_per_scene:
            continue

        for use_no in range(already_used, max_uses_per_scene):
            slot_scene_indices.append(scene_idx)
            slot_use_numbers.append(use_no)

    if not slot_scene_indices:
        log_fn("Nenhum slot de cena disponível para casar com as frases.")
        return [], set(), set()

    slot_scene_indices_arr = np.array(slot_scene_indices, dtype=np.int32)
    slot_use_numbers_arr = np.array(slot_use_numbers, dtype=np.int32)
    slot_decay_factors = np.power(
        np.float32(SCENE_REUSE_SCORE_DECAY),
        slot_use_numbers_arr,
    ).astype(np.float32)

    # Cada slot representa um possível uso daquela cena.
    # Se max_uses_per_scene=3, a mesma cena pode virar 3 colunas diferentes.
    score_matrix = (
        base_score_matrix[:, slot_scene_indices_arr]
        * slot_decay_factors[np.newaxis, :]
    ).astype(np.float32)

    valid_mask = score_matrix >= effective_min_similarity
    valid_edges = int(valid_mask.sum())

    if valid_edges <= 0:
        log_fn(
            f"Nenhuma combinação frase/cena passou do limiar {effective_min_similarity:.3f}."
        )
        return [], set(), set()

    log_fn(
        f"Otimizando globalmente {len(script_items)} frase(s), "
        f"{len(scene_descs)} cena(s), {len(slot_scene_indices)} slot(s) de uso e "
        f"{valid_edges} combinação(ões) válidas..."
    )

    score_int_matrix = np.rint(score_matrix * SCORE_SCALE).astype(np.int64)
    max_score_int = int(score_int_matrix[valid_mask].max())

    # Nota mínima para valer mais a pena usar uma cena
    # do que deixar a frase sem cena.
    min_assign_score_int = int(round(effective_min_assign_score * SCORE_SCALE))

    # "Âncora" para manter todos os custos não negativos.
    cost_anchor_int = max(max_score_int, min_assign_score_int)

    # Custo da opção "sem cena".
    # Se a cena tiver score abaixo de MIN_ASSIGN_SCORE,
    # o dummy ("sem cena") fica melhor.
    unmatched_cost = cost_anchor_int - min_assign_score_int

    # Custo bem alto para combinações inválidas.
    invalid_cost = (cost_anchor_int + SCORE_SCALE) * 1000

    n_phrases = len(script_items)
    n_slots = len(slot_scene_indices)

    # Matriz retangular:
    # - colunas [0:n_slots]      -> slots reais de cena
    # - colunas [n_slots:...]    -> colunas dummy (frase sem cena)
    cost_matrix = np.full(
        (n_phrases, n_slots + n_phrases),
        unmatched_cost,
        dtype=np.int64,
    )

    # Quanto maior o score, menor o custo.
    real_slot_costs = cost_anchor_int - score_int_matrix
    cost_matrix[:, :n_slots] = np.where(
        valid_mask, real_slot_costs, invalid_cost
    )

    min_accept_score = max(effective_min_similarity,
                           effective_min_assign_score)

    def _collect_assignments_from_columns(
        chosen_cols: List[int],
    ) -> Tuple[List[Assignment], Set[int], Set[int]]:
        assignments: List[Assignment] = []
        used_phrases: Set[int] = set()
        scene_use_counts = {
            int(k): int(v)
            for k, v in initial_scene_use_counts.items()
            if int(v) > 0
        }

        for phrase_idx, col_idx in enumerate(chosen_cols):
            if col_idx is None:
                continue

            if col_idx < 0 or col_idx >= n_slots:
                # coluna dummy = frase fica sem cena
                continue

            if not valid_mask[phrase_idx, col_idx]:
                continue

            score = float(score_matrix[phrase_idx, col_idx])
            if score < min_accept_score:
                continue

            scene_idx = int(slot_scene_indices_arr[col_idx])
            assignments.append(
                Assignment(
                    phrase_idx=phrase_idx,
                    scene_idx=scene_idx,
                    score=score,
                )
            )
            used_phrases.add(phrase_idx)
            scene_use_counts[scene_idx] = scene_use_counts.get(
                scene_idx, 0) + 1

        assignments.sort(key=lambda a: a.phrase_idx)
        used_scenes = {
            si for si, count in scene_use_counts.items()
            if count > 0
        }
        return assignments, used_phrases, used_scenes

    # -----------------------------
    # Tentativa 1: resolvedor exato rápido (SciPy)
    # -----------------------------
    try:
        from scipy.optimize import linear_sum_assignment

        row_ind, col_ind = linear_sum_assignment(cost_matrix)
        chosen_cols = [None] * n_phrases
        for r, c in zip(row_ind.tolist(), col_ind.tolist()):
            chosen_cols[int(r)] = int(c)

        log_fn("Otimização global exata concluída com SciPy.")
        return _collect_assignments_from_columns(chosen_cols)

    except Exception as scipy_err:
        log_fn(
            "SciPy não disponível (ou falhou). "
            "Vou usar fallback exato em Python puro. "
            f"Motivo: {scipy_err}"
        )

    # -----------------------------
    # Tentativa 2: min-cost max-flow exato em Python puro
    # -----------------------------
    import heapq

    def _add_edge(graph, u: int, v: int, cap: int, cost: int):
        graph[u].append([v, len(graph[v]), cap, cost])
        graph[v].append([u, len(graph[u]) - 1, 0, -cost])
        return graph[u][-1]

    source = 0
    phrase_offset = 1
    slot_offset = phrase_offset + n_phrases
    sink = slot_offset + n_slots
    node_count = sink + 1

    graph = [[] for _ in range(node_count)]
    phrase_slot_edges = [[] for _ in range(n_phrases)]

    for phrase_idx in range(n_phrases):
        phrase_node = phrase_offset + phrase_idx
        _add_edge(graph, source, phrase_node, 1, 0)

        # opção dummy: frase pode terminar sem cena
        _add_edge(graph, phrase_node, sink, 1, unmatched_cost)

        valid_slot_cols = np.flatnonzero(valid_mask[phrase_idx])
        for col_idx in valid_slot_cols.tolist():
            slot_node = slot_offset + col_idx
            edge = _add_edge(
                graph,
                phrase_node,
                slot_node,
                1,
                int(real_slot_costs[phrase_idx, col_idx]),
            )
            phrase_slot_edges[phrase_idx].append((col_idx, edge))

    for col_idx in range(n_slots):
        slot_node = slot_offset + col_idx
        _add_edge(graph, slot_node, sink, 1, 0)

    target_flow = n_phrases
    flow = 0
    potentials = [0] * node_count
    total_cost = 0

    while flow < target_flow:
        dist = [10**18] * node_count
        parent = [None] * node_count
        dist[source] = 0
        pq = [(0, source)]

        while pq:
            cur_dist, u = heapq.heappop(pq)
            if cur_dist != dist[u]:
                continue
            if u == sink:
                break

            for edge_idx, edge in enumerate(graph[u]):
                v, rev, cap, cost = edge
                if cap <= 0:
                    continue

                reduced_cost = cost + potentials[u] - potentials[v]
                nd = cur_dist + reduced_cost
                if nd < dist[v]:
                    dist[v] = nd
                    parent[v] = (u, edge_idx)
                    heapq.heappush(pq, (nd, v))

        if parent[sink] is None:
            break

        for i in range(node_count):
            if dist[i] < 10**18:
                potentials[i] += dist[i]

        add_flow = target_flow - flow
        v = sink
        while v != source:
            u, edge_idx = parent[v]
            add_flow = min(add_flow, graph[u][edge_idx][2])
            v = u

        v = sink
        while v != source:
            u, edge_idx = parent[v]
            edge = graph[u][edge_idx]
            rev_idx = edge[1]
            edge[2] -= add_flow
            graph[v][rev_idx][2] += add_flow
            total_cost += edge[3] * add_flow
            v = u

        flow += add_flow

    chosen_cols = [None] * n_phrases
    for phrase_idx in range(n_phrases):
        for col_idx, edge in phrase_slot_edges[phrase_idx]:
            # Quando a aresta é usada, a capacidade vai de 1 para 0.
            if edge[2] == 0:
                chosen_cols[phrase_idx] = col_idx
                break

    log_fn("Otimização global exata concluída com fallback Python puro.")
    return _collect_assignments_from_columns(chosen_cols)


def _ffprobe_duration_seconds(path: str) -> Optional[float]:
    """
    Retorna a duração do vídeo em segundos usando ffprobe.
    Se não tiver ffprobe, retorna None (e aí não recorta).
    """
    if shutil.which("ffprobe") is None:
        return None

    try:
        cmd = [
            "ffprobe",
            "-v", "error",
            "-show_entries", "format=duration",
            "-of", "default=noprint_wrappers=1:nokey=1",
            path,
        ]
        r = subprocess.run(cmd, capture_output=True, text=True)
        if r.returncode != 0:
            return None
        s = (r.stdout or "").strip()
        return float(s) if s else None
    except Exception:
        return None


def _compute_center_clip_window(duration_s: float, clip_len_s: float) -> Tuple[float, float]:
    """
    Retorna (start, length). Se o vídeo for <= clip_len_s, retorna (0, duration_s).
    """
    if duration_s <= clip_len_s:
        return 0.0, float(duration_s)
    start = (duration_s - clip_len_s) / 2.0
    return max(0.0, float(start)), float(clip_len_s)


def _format_ffmpeg_error(cmd: List[str], result: Optional[subprocess.CompletedProcess] = None, exc: Optional[Exception] = None) -> str:
    """
    Monta uma mensagem curta e útil para logar o motivo da falha do ffmpeg.

    stdout = saída padrão do programa.
    stderr = saída de erro do programa (geralmente onde o ffmpeg explica a falha).
    returncode = código de saída; 0 normalmente significa sucesso.
    """
    cmd_str = " ".join(f'"{p}"' if " " in str(p) else str(p) for p in cmd)

    if exc is not None:
        return f"Falha ao executar ffmpeg: {exc} | comando: {cmd_str}"

    details: List[str] = []
    if result is not None:
        if result.returncode not in (None, 0):
            details.append(f"returncode={result.returncode}")

        stderr_text = (result.stderr or "").strip()
        stdout_text = (result.stdout or "").strip()
        raw_text = stderr_text or stdout_text

        if raw_text:
            lines = [line.strip()
                     for line in raw_text.splitlines() if line.strip()]
            if lines:
                tail = " | ".join(lines[-3:])
                details.append(f"detalhe={tail}")

    if not details:
        details.append("ffmpeg falhou sem detalhe no stdout/stderr")

    return f"{' ; '.join(details)} | comando: {cmd_str}"


def _ffmpeg_extract_clip_to_temp(
    in_path: str,
    start_s: float,
    duration_s: float,
    include_audio: bool = True,
) -> Tuple[Optional[str], Optional[str]]:
    """
    Recorta um trecho do vídeo com ffmpeg e salva num arquivo temporário (.mp4).

    Retorna:
      - caminho do arquivo temporário, se der certo
      - mensagem de erro amigável para log, se der errado
    """
    if shutil.which("ffmpeg") is None:
        return None, "ffmpeg não foi encontrado no sistema (não está instalado ou não está no PATH)."

    cmd = []
    tmp_path = None
    try:
        tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".mp4")
        tmp_path = tmp.name
        tmp.close()

        # Re-encode para ser robusto (cópia direta pode falhar por keyframe)
        cmd = [
            "ffmpeg", "-y",
            "-ss", f"{start_s:.3f}",
            "-i", in_path,
            "-t", f"{duration_s:.3f}",
            "-map", "0:v:0?",
        ]

        if include_audio:
            cmd += ["-map", "0:a:0?", "-c:a", "aac"]
        else:
            cmd += ["-an"]

        cmd += [
            "-c:v", "libx264",
            "-preset", "veryfast",
            "-crf", "23",
            "-movflags", "+faststart",
            tmp_path,
        ]

        r = subprocess.run(cmd, capture_output=True,
                           text=True, errors="replace")
        if r.returncode != 0:
            if tmp_path and os.path.exists(tmp_path):
                try:
                    os.remove(tmp_path)
                except Exception:
                    pass
            return None, _format_ffmpeg_error(cmd, result=r)

        if (not os.path.exists(tmp_path)) or os.path.getsize(tmp_path) == 0:
            if tmp_path and os.path.exists(tmp_path):
                try:
                    os.remove(tmp_path)
                except Exception:
                    pass
            return None, "ffmpeg terminou sem gerar arquivo válido (arquivo ausente ou vazio)."

        return tmp_path, None
    except Exception as exc:
        if tmp_path and os.path.exists(tmp_path):
            try:
                os.remove(tmp_path)
            except Exception:
                pass
        return None, _format_ffmpeg_error(cmd, exc=exc)


def _ffmpeg_remove_audio_to_temp(in_path: str) -> Tuple[Optional[str], Optional[str]]:
    """
    Remove o áudio do vídeo e salva num arquivo temporário (.mp4).
    Re-encode do vídeo (libx264) pra ser mais robusto entre formatos.

    Retorna:
      - caminho do arquivo temporário, se der certo
      - mensagem de erro amigável para log, se der errado
    """
    if shutil.which("ffmpeg") is None:
        return None, "ffmpeg não foi encontrado no sistema (não está instalado ou não está no PATH)."

    cmd = []
    tmp_path = None
    try:
        tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".mp4")
        tmp_path = tmp.name
        tmp.close()

        cmd = [
            "ffmpeg", "-y",
            "-i", in_path,
            "-map", "0:v:0?",
            "-an",
            "-c:v", "libx264",
            "-preset", "veryfast",
            "-crf", "23",
            "-movflags", "+faststart",
            tmp_path,
        ]

        r = subprocess.run(cmd, capture_output=True,
                           text=True, errors="replace")
        if r.returncode != 0:
            if tmp_path and os.path.exists(tmp_path):
                try:
                    os.remove(tmp_path)
                except Exception:
                    pass
            return None, _format_ffmpeg_error(cmd, result=r)

        if (not os.path.exists(tmp_path)) or os.path.getsize(tmp_path) == 0:
            if tmp_path and os.path.exists(tmp_path):
                try:
                    os.remove(tmp_path)
                except Exception:
                    pass
            return None, "ffmpeg terminou sem gerar arquivo válido (arquivo ausente ou vazio)."

        return tmp_path, None
    except Exception as exc:
        if tmp_path and os.path.exists(tmp_path):
            try:
                os.remove(tmp_path)
            except Exception:
                pass
        return None, _format_ffmpeg_error(cmd, exc=exc)


class ProcessingCancelled(Exception):
    """Usada para interromper o worker quando o usuário fecha a janela de log."""
    pass

# ---------------------------
# GUI (Tkinter)
# ---------------------------


class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Renomeador de Cenas (Gemini + Matching Global)")
        self.geometry("1180x760")

        self.selected_files: List[str] = []
        self.roteiro_text: str = ""
        self.worker_thread: Optional[threading.Thread] = None
        self.q: "queue.Queue[str]" = queue.Queue()
        self.stop_event = threading.Event()

        self.log_window: Optional[tk.Toplevel] = None
        self.log_box_popup: Optional[tk.Text] = None
        self.log_cancel_button: Optional[ttk.Button] = None
        self.log_close_button: Optional[ttk.Button] = None
        self.processing_active = False
        self.processing_finished = False

        self.update_notes_window: Optional[tk.Toplevel] = None
        self.update_notes_textbox: Optional[tk.Text] = None

        self.matching_settings_window: Optional[tk.Toplevel] = None
        self.reprocess_settings_window: Optional[tk.Toplevel] = None

        # Variáveis de configuração
        self.api_key_var = tk.StringVar(value="")
        self.words_var = tk.IntVar(value=7)
        self.allow_pro_var = tk.BooleanVar(value=True)
        self.include_audio_var = tk.BooleanVar(value=False)
        self.use_script_var = tk.BooleanVar(value=True)
        self.out_dir_var = tk.StringVar(value="")
        self.sentences_per_chunk_var = tk.IntVar(value=1)
        self.split_long_phrases_var = tk.BooleanVar(value=False)
        self.split_words_threshold_var = tk.IntVar(value=10)

        self.allow_scene_reuse_var = tk.BooleanVar(value=False)
        self.max_reuse_per_scene_var = tk.IntVar(value=2)

        self.create_green_placeholders_var = tk.BooleanVar(value=True)

        # novas configs do matching
        self.min_similarity_var = tk.DoubleVar(value=MIN_SIMILARITY)
        self.min_assign_score_var = tk.DoubleVar(value=MIN_ASSIGN_SCORE)
        self.scene_reuse_score_decay_var = tk.DoubleVar(
            value=SCENE_REUSE_SCORE_DECAY)

        self.reprocess_min_assign_score_factor_var = tk.DoubleVar(
            value=REPROCESS_MIN_ASSIGN_SCORE_FACTOR
        )

        self._build_ui()
        self._build_top_menu()

        self.protocol("WM_DELETE_WINDOW", self._on_close)

        try:
            clear_stable_match_cache_file()
        except Exception:
            pass

        cfg = load_config()

        if cfg.get("gemini_api_key"):
            self.api_key_var.set(cfg["gemini_api_key"])
        if cfg.get("words"):
            self.words_var.set(int(cfg["words"]))
        if cfg.get("out_dir"):
            self.out_dir_var.set(cfg["out_dir"])
        if cfg.get("sentences_per_chunk"):
            self.sentences_per_chunk_var.set(int(cfg["sentences_per_chunk"]))
        if cfg.get("allow_pro_fallback") is not None:
            self.allow_pro_var.set(bool(cfg["allow_pro_fallback"]))
        if cfg.get("include_video_audio") is not None:
            self.include_audio_var.set(bool(cfg["include_video_audio"]))
        if cfg.get("use_script_matching") is not None:
            self.use_script_var.set(bool(cfg["use_script_matching"]))
        if cfg.get("split_long_phrases") is not None:
            self.split_long_phrases_var.set(bool(cfg["split_long_phrases"]))
        if cfg.get("split_words_threshold") is not None:
            self.split_words_threshold_var.set(
                int(cfg["split_words_threshold"]))

        if cfg.get("allow_scene_reuse") is not None:
            self.allow_scene_reuse_var.set(bool(cfg["allow_scene_reuse"]))
        if cfg.get("max_reuse_per_scene") is not None:
            self.max_reuse_per_scene_var.set(int(cfg["max_reuse_per_scene"]))

        if cfg.get("create_green_placeholders") is not None:
            self.create_green_placeholders_var.set(
                bool(cfg["create_green_placeholders"]))

        if cfg.get("min_similarity") is not None:
            self.min_similarity_var.set(float(cfg["min_similarity"]))

        if cfg.get("min_assign_score") is not None:
            self.min_assign_score_var.set(float(cfg["min_assign_score"]))

        if cfg.get("scene_reuse_score_decay") is not None:
            self.scene_reuse_score_decay_var.set(
                float(cfg["scene_reuse_score_decay"]))

        if cfg.get("reprocess_min_assign_score_factor") is not None:
            self.reprocess_min_assign_score_factor_var.set(
                float(cfg["reprocess_min_assign_score_factor"])
            )

        self._apply_runtime_matching_settings()

        self._poll_queue()

    def _on_close(self):
        try:
            self._save_current_config()
        except Exception:
            pass

        self.stop_event.set()

        if self.log_window and self.log_window.winfo_exists():
            try:
                self.log_window.destroy()
            except Exception:
                pass

        if self.update_notes_window and self.update_notes_window.winfo_exists():
            try:
                self.update_notes_window.destroy()
            except Exception:
                pass

        if self.matching_settings_window and self.matching_settings_window.winfo_exists():
            try:
                self.matching_settings_window.destroy()
            except Exception:
                pass

        if self.reprocess_settings_window and self.reprocess_settings_window.winfo_exists():
            try:
                self.reprocess_settings_window.destroy()
            except Exception:
                pass

        try:
            self.destroy()
        except Exception:
            pass

    def _build_ui(self):
        self.main_root = ttk.Frame(self, padding=12)
        self.main_root.pack(fill="both", expand=True)

        # -------------------------
        # 1) Cenas
        # -------------------------
        files_frame = ttk.LabelFrame(
            self.main_root, text="1) Cenas (imagens e/ou vídeos)", padding=10
        )
        files_frame.pack(fill="x")

        btns = ttk.Frame(files_frame)
        btns.pack(fill="x")

        ttk.Button(btns, text="Adicionar arquivos",
                   command=self.add_files).pack(side="left")
        ttk.Button(btns, text="Remover selecionado",
                   command=self.remove_selected).pack(side="left", padx=6)
        ttk.Button(btns, text="Limpar lista",
                   command=self.clear_files).pack(side="left")

        self.files_list = tk.Listbox(
            files_frame, height=8, selectmode=tk.EXTENDED
        )
        self.files_list.pack(fill="x", pady=8)

        # -------------------------
        # Área do meio: esquerda = roteiro | direita = configurações
        # -------------------------
        middle_area = ttk.Frame(self.main_root)
        middle_area.pack(fill="both", expand=True, pady=10)

        left_col = ttk.Frame(middle_area)
        left_col.pack(side="left", fill="both", expand=True)

        right_col = ttk.Frame(middle_area, width=420)
        right_col.pack(side="right", fill="y", padx=(6, 0))
        right_col.pack_propagate(False)

        # -------------------------
        # 2) Roteiro
        # -------------------------
        roteiro_frame = ttk.LabelFrame(
            left_col, text="2) Roteiro (.txt)", padding=10
        )
        roteiro_frame.pack(fill="both", expand=True)

        roteiro_top = ttk.Frame(roteiro_frame)
        roteiro_top.pack(fill="x")

        ttk.Button(
            roteiro_top,
            text="Abrir roteiro.txt",
            command=self.load_roteiro
        ).pack(side="left")

        ttk.Checkbutton(
            roteiro_top,
            text="Usar roteiro para matching/renomear",
            variable=self.use_script_var
        ).pack(side="left", padx=12)

        self.roteiro_info = ttk.Label(
            roteiro_top, text="Nenhum roteiro carregado"
        )
        self.roteiro_info.pack(side="left", padx=10)

        # altura reduzida aqui
        self.roteiro_box = tk.Text(
            roteiro_frame,
            height=7,
            wrap="word"
        )
        self.roteiro_box.pack(fill="both", expand=True, pady=8)
        self.roteiro_box.insert(
            "1.0", "Abra um roteiro .txt para aparecer aqui."
        )
        self.roteiro_box.configure(state="disabled")

        # -------------------------
        # 3) Configurações
        # -------------------------
        cfg_frame = ttk.LabelFrame(
            right_col, text="3) Configurações", padding=10
        )
        cfg_frame.pack(fill="both", expand=True)

        row0 = ttk.Frame(cfg_frame)
        row0.pack(fill="x", pady=(0, 6))
        ttk.Label(
            row0,
            text='API do Gemini, fallback PRO e áudio do vídeo ficam no menu "Configurações".',
            wraplength=260,
            justify="left",
        ).pack(side="left")

        row1 = ttk.Frame(cfg_frame)
        row1.pack(fill="x", pady=6)
        ttk.Label(row1, text="Modelos:", width=18,
                  anchor="w").pack(side="left")
        ttk.Label(
            row1,
            text=(
                f"Imagem: {GEMINI_IMAGE_MODEL_CHEAP}\n"
                f"Fallback PRO: {GEMINI_IMAGE_MODEL_PRO}\n"
                f"Alternativo: {GEMINI_IMAGE_MODEL_MID}\n"
                f"Vídeo: {GEMINI_VIDEO_MODEL}"
            ),
            justify="left",
            wraplength=320,
        ).pack(side="left", padx=8)

        row2 = ttk.Frame(cfg_frame)
        row2.pack(fill="x", pady=6)
        ttk.Label(
            row2,
            text="Qtd. palavras no nome:",
            width=22,
            anchor="w"
        ).pack(side="left")
        ttk.Spinbox(
            row2,
            from_=5,
            to=10,
            textvariable=self.words_var,
            width=6
        ).pack(side="left", padx=6)

        row3 = ttk.Frame(cfg_frame)
        row3.pack(fill="x", pady=6)
        ttk.Label(
            row3,
            text="Pasta de saída:",
            width=22,
            anchor="w"
        ).pack(side="left")
        ttk.Entry(
            row3,
            textvariable=self.out_dir_var,
            width=28
        ).pack(side="left", padx=6)
        ttk.Button(
            row3,
            text="Escolher",
            command=self.pick_out_dir
        ).pack(side="left")

        row4 = ttk.Frame(cfg_frame)
        row4.pack(fill="x", pady=6)
        ttk.Label(
            row4,
            text="Frases por item:",
            width=22,
            anchor="w"
        ).pack(side="left")
        ttk.Spinbox(
            row4,
            from_=1,
            to=10,
            increment=1,
            textvariable=self.sentences_per_chunk_var,
            width=6,
        ).pack(side="left", padx=6)

        row5 = ttk.Frame(cfg_frame)
        row5.pack(fill="x", pady=6)

        ttk.Checkbutton(
            row5,
            text="Criar cenas intermediárias em frases longas",
            variable=self.split_long_phrases_var,
        ).pack(anchor="w")

        sub_row5 = ttk.Frame(cfg_frame)
        sub_row5.pack(fill="x", pady=(0, 6))
        ttk.Label(
            sub_row5,
            text="Limite de palavras:",
            width=22,
            anchor="w"
        ).pack(side="left")
        ttk.Spinbox(
            sub_row5,
            from_=5,
            to=80,
            increment=1,
            textvariable=self.split_words_threshold_var,
            width=6,
        ).pack(side="left", padx=6)

        row6 = ttk.Frame(cfg_frame)
        row6.pack(fill="x", pady=6)

        ttk.Checkbutton(
            row6,
            text="Permitir repetir a mesma cena em mais de uma frase",
            variable=self.allow_scene_reuse_var,
        ).pack(anchor="w")

        sub_row6 = ttk.Frame(cfg_frame)
        sub_row6.pack(fill="x", pady=(0, 6))
        ttk.Label(
            sub_row6,
            text="Máx. usos por cena:",
            width=22,
            anchor="w"
        ).pack(side="left")
        ttk.Spinbox(
            sub_row6,
            from_=2,
            to=10,
            increment=1,
            textvariable=self.max_reuse_per_scene_var,
            width=6,
        ).pack(side="left", padx=6)

        row7 = ttk.Frame(cfg_frame)
        row7.pack(fill="x", pady=6)

        ttk.Checkbutton(
            row7,
            text="Criar imagem verde para frases sem cena",
            variable=self.create_green_placeholders_var,
        ).pack(anchor="w")

        # -------------------------
        # Ação + Progresso
        # -------------------------
        action = ttk.Frame(self.main_root)
        action.pack(fill="x", pady=10)

        self.run_btn = ttk.Button(
            action, text="Enviar e renomear", command=self.run
        )
        self.run_btn.pack(side="left")

        self.reprocess_btn = ttk.Button(
            action,
            text="Reprocessar pendências",
            command=self.run_reprocess_pending,
        )
        self.reprocess_btn.pack(side="left", padx=(6, 0))

        self.undo_btn = ttk.Button(
            action,
            text="Desfazer última execução",
            command=self.undo_last_run,
        )
        self.undo_btn.pack(side="left", padx=(6, 0))

        self.progress = ttk.Progressbar(action, mode="determinate")
        self.progress.pack(side="left", fill="x", expand=True, padx=10)

    def _build_top_menu(self):
        self.menubar = tk.Menu(self)

        self.config_menu = tk.Menu(self.menubar, tearoff=0)
        self.config_menu.add_command(
            label="API do Gemini...",
            command=self._ask_gemini_api_key
        )
        self.config_menu.add_command(
            label="Configurações do matching...",
            command=self._open_matching_settings_window
        )
        self.config_menu.add_command(
            label="Configuração de Reprocessamento...",
            command=self._open_reprocess_settings_window
        )
        self.config_menu.add_separator()
        self.config_menu.add_checkbutton(
            label="Permitir fallback PRO",
            variable=self.allow_pro_var,
            onvalue=True,
            offvalue=False,
            command=self._save_current_config,
        )
        self.config_menu.add_checkbutton(
            label="Enviar vídeo com áudio",
            variable=self.include_audio_var,
            onvalue=True,
            offvalue=False,
            command=self._save_current_config,
        )

        self.help_menu = tk.Menu(self.menubar, tearoff=0)
        self.help_menu.add_command(
            label="Notas de atualização",
            command=self._open_update_notes_window,
        )

        self.menubar.add_cascade(label="Configurações", menu=self.config_menu)
        self.menubar.add_cascade(label="Ajuda", menu=self.help_menu)

        self.config(menu=self.menubar)

    def _ask_gemini_api_key(self):
        value = simpledialog.askstring(
            "API do Gemini",
            "Cole sua GEMINI_API_KEY.\n"
            "Se deixar vazio, o programa tenta usar a variável de ambiente GEMINI_API_KEY.",
            initialvalue=self.api_key_var.get(),
            show="*",
            parent=self,
        )
        if value is not None:
            self.api_key_var.set(value.strip())
            self._save_current_config()

    def _apply_runtime_matching_settings(self):
        global MIN_SIMILARITY, MIN_ASSIGN_SCORE, SCENE_REUSE_SCORE_DECAY
        global REPROCESS_MIN_ASSIGN_SCORE_FACTOR

        MIN_SIMILARITY = float(self.min_similarity_var.get())
        MIN_ASSIGN_SCORE = float(self.min_assign_score_var.get())
        SCENE_REUSE_SCORE_DECAY = float(self.scene_reuse_score_decay_var.get())
        REPROCESS_MIN_ASSIGN_SCORE_FACTOR = float(
            self.reprocess_min_assign_score_factor_var.get()
        )

    def _open_matching_settings_window(self):
        if self.matching_settings_window and self.matching_settings_window.winfo_exists():
            self.matching_settings_window.deiconify()
            self.matching_settings_window.lift()
            self.matching_settings_window.focus_force()
            return

        self.matching_settings_window = tk.Toplevel(self)
        self.matching_settings_window.title("Configurações do matching")
        self.matching_settings_window.geometry("560x350")
        self.matching_settings_window.transient(self)

        root = ttk.Frame(self.matching_settings_window, padding=12)
        root.pack(fill="both", expand=True)

        ttk.Label(
            root,
            text="Ajustes de coerência entre frases e cenas",
            font=("Segoe UI", 10, "bold"),
        ).pack(anchor="w", pady=(0, 10))

        ttk.Label(
            root,
            text=(
                "MIN_SIMILARITY = filtro inicial.\n"
                "MIN_ASSIGN_SCORE = nota mínima para aceitar renomear a cena.\n"
                "SCENE_REUSE_SCORE_DECAY = quanto a nota cai quando a mesma cena se repete."
            ),
            justify="left",
            wraplength=520,
        ).pack(anchor="w", pady=(0, 12))

        row1 = ttk.Frame(root)
        row1.pack(fill="x", pady=6)
        ttk.Label(row1, text="MIN_SIMILARITY:",
                  width=24, anchor="w").pack(side="left")
        ttk.Entry(row1, textvariable=self.min_similarity_var,
                  width=12).pack(side="left", padx=6)
        ttk.Label(row1, text="Ex.: 0.30", anchor="w").pack(side="left")

        row2 = ttk.Frame(root)
        row2.pack(fill="x", pady=6)
        ttk.Label(row2, text="MIN_ASSIGN_SCORE:",
                  width=24, anchor="w").pack(side="left")
        ttk.Entry(row2, textvariable=self.min_assign_score_var,
                  width=12).pack(side="left", padx=6)
        ttk.Label(row2, text="Ex.: 0.82", anchor="w").pack(side="left")

        row3 = ttk.Frame(root)
        row3.pack(fill="x", pady=6)
        ttk.Label(row3, text="SCENE_REUSE_SCORE_DECAY:",
                  width=24, anchor="w").pack(side="left")
        ttk.Entry(row3, textvariable=self.scene_reuse_score_decay_var,
                  width=12).pack(side="left", padx=6)
        ttk.Label(row3, text="Ex.: 0.92", anchor="w").pack(side="left")

        ttk.Separator(root).pack(fill="x", pady=12)

        tip_box = ttk.Frame(root)
        tip_box.pack(fill="x", pady=(0, 12))

        ttk.Label(
            tip_box,
            text=(
                "Sugestão prática:\n"
                "- mais rígido:  MIN_SIMILARITY=0.30 | MIN_ASSIGN_SCORE=0.48 | DECAY=0.80\n"
                "- mais solto:   MIN_SIMILARITY=0.24 | MIN_ASSIGN_SCORE=0.40 | DECAY=0.92"
            ),
            justify="left",
            wraplength=520,
        ).pack(anchor="w")

        btns = ttk.Frame(root)
        btns.pack(fill="x")

        ttk.Button(
            btns,
            text="Salvar",
            command=self._save_matching_settings_from_window,
        ).pack(side="right")

        ttk.Button(
            btns,
            text="Cancelar",
            command=self._close_matching_settings_window,
        ).pack(side="right", padx=(0, 6))

        self.matching_settings_window.protocol(
            "WM_DELETE_WINDOW",
            self._close_matching_settings_window
        )

        self.matching_settings_window.lift()
        self.matching_settings_window.focus_force()

    def _save_matching_settings_from_window(self):
        try:
            min_similarity = float(self.min_similarity_var.get())
            min_assign_score = float(self.min_assign_score_var.get())
            reuse_decay = float(self.scene_reuse_score_decay_var.get())
        except Exception:
            messagebox.showerror(
                "Valores inválidos",
                "Preencha os 3 campos com números válidos.\n\nExemplo: 0.30, 0.48, 0.80",
                parent=self.matching_settings_window or self,
            )
            return

        if not (0.0 <= min_similarity <= 1.0):
            messagebox.showerror(
                "Valor inválido",
                "MIN_SIMILARITY deve ficar entre 0.0 e 1.0",
                parent=self.matching_settings_window or self,
            )
            return

        if not (0.0 <= min_assign_score <= 1.0):
            messagebox.showerror(
                "Valor inválido",
                "MIN_ASSIGN_SCORE deve ficar entre 0.0 e 1.0",
                parent=self.matching_settings_window or self,
            )
            return

        if not (0.0 <= reuse_decay <= 1.0):
            messagebox.showerror(
                "Valor inválido",
                "SCENE_REUSE_SCORE_DECAY deve ficar entre 0.0 e 1.0",
                parent=self.matching_settings_window or self,
            )
            return

        if min_assign_score < min_similarity:
            messagebox.showerror(
                "Configuração inconsistente",
                "MIN_ASSIGN_SCORE deve ser maior ou igual a MIN_SIMILARITY.\n\n"
                "Senão o filtro final fica mais fraco que o filtro inicial.",
                parent=self.matching_settings_window or self,
            )
            return

        self.min_similarity_var.set(min_similarity)
        self.min_assign_score_var.set(min_assign_score)
        self.scene_reuse_score_decay_var.set(reuse_decay)

        self._apply_runtime_matching_settings()
        self._save_current_config()

        messagebox.showinfo(
            "Configurações salvas",
            "As configurações do matching foram salvas e já estão valendo para a próxima execução.",
            parent=self.matching_settings_window or self,
        )

        self._close_matching_settings_window()

    def _close_matching_settings_window(self):
        if self.matching_settings_window and self.matching_settings_window.winfo_exists():
            try:
                self.matching_settings_window.destroy()
            except Exception:
                pass

        self.matching_settings_window = None

    def _open_reprocess_settings_window(self):
        if self.reprocess_settings_window and self.reprocess_settings_window.winfo_exists():
            self.reprocess_settings_window.deiconify()
            self.reprocess_settings_window.lift()
            self.reprocess_settings_window.focus_force()
            return

        self.reprocess_settings_window = tk.Toplevel(self)
        self.reprocess_settings_window.title("Configuração de Reprocessamento")
        self.reprocess_settings_window.geometry("560x300")
        self.reprocess_settings_window.transient(self)

        root = ttk.Frame(self.reprocess_settings_window, padding=12)
        root.pack(fill="both", expand=True)

        ttk.Label(
            root,
            text="Ajustes do reprocessamento de pendências",
            font=("Segoe UI", 10, "bold"),
        ).pack(anchor="w", pady=(0, 10))

        ttk.Label(
            root,
            text=(
                "REPROCESS_MIN_ASSIGN_SCORE_FACTOR = fator usado para soltar "
                "ou manter mais rígido o critério de aceitação no botão "
                "\"Reprocessar pendências\".\n\n"
                "Exemplos:\n"
                "- 1.00 = não afrouxa nada\n"
                "- 0.95 = afrouxa bem pouco\n"
                "- 0.70 = afrouxa bastante a cada nova tentativa"
            ),
            justify="left",
            wraplength=520,
        ).pack(anchor="w", pady=(0, 12))

        row1 = ttk.Frame(root)
        row1.pack(fill="x", pady=6)
        ttk.Label(
            row1,
            text="REPROCESS_MIN_ASSIGN_SCORE_FACTOR:",
            width=34,
            anchor="w",
        ).pack(side="left")
        ttk.Entry(
            row1,
            textvariable=self.reprocess_min_assign_score_factor_var,
            width=12,
        ).pack(side="left", padx=6)
        ttk.Label(row1, text="Ex.: 0.95", anchor="w").pack(side="left")

        ttk.Separator(root).pack(fill="x", pady=12)

        ttk.Label(
            root,
            text="Recomendação prática: usar entre 0.70 e 0.95.",
            justify="left",
            wraplength=520,
        ).pack(anchor="w", pady=(0, 12))

        btns = ttk.Frame(root)
        btns.pack(fill="x")

        ttk.Button(
            btns,
            text="Salvar",
            command=self._save_reprocess_settings_from_window,
        ).pack(side="right")

        ttk.Button(
            btns,
            text="Cancelar",
            command=self._close_reprocess_settings_window,
        ).pack(side="right", padx=(0, 6))

        self.reprocess_settings_window.protocol(
            "WM_DELETE_WINDOW",
            self._close_reprocess_settings_window
        )

        self.reprocess_settings_window.lift()
        self.reprocess_settings_window.focus_force()

    def _save_reprocess_settings_from_window(self):
        try:
            factor = float(self.reprocess_min_assign_score_factor_var.get())
        except Exception:
            messagebox.showerror(
                "Valor inválido",
                "Preencha o campo com um número válido.\n\nExemplo: 0.95",
                parent=self.reprocess_settings_window or self,
            )
            return

        if not (0.0 < factor <= 1.0):
            messagebox.showerror(
                "Valor inválido",
                "REPROCESS_MIN_ASSIGN_SCORE_FACTOR deve ser maior que 0.0 e menor ou igual a 1.0.",
                parent=self.reprocess_settings_window or self,
            )
            return

        self.reprocess_min_assign_score_factor_var.set(factor)

        self._apply_runtime_matching_settings()
        self._save_current_config()

        messagebox.showinfo(
            "Configuração salva",
            "A configuração de reprocessamento foi salva e já vale para a próxima tentativa.",
            parent=self.reprocess_settings_window or self,
        )

        self._close_reprocess_settings_window()

    def _close_reprocess_settings_window(self):
        if self.reprocess_settings_window and self.reprocess_settings_window.winfo_exists():
            try:
                self.reprocess_settings_window.destroy()
            except Exception:
                pass

        self.reprocess_settings_window = None

    def _open_update_notes_window(self):
        if self.update_notes_window and self.update_notes_window.winfo_exists():
            self.update_notes_window.deiconify()
            self.update_notes_window.lift()
            self.update_notes_window.focus_force()
            return

        self.update_notes_window = tk.Toplevel(self)
        self.update_notes_window.title("Notas de atualização")
        self.update_notes_window.geometry("860x620")
        self.update_notes_window.transient(self)

        root = ttk.Frame(self.update_notes_window, padding=12)
        root.pack(fill="both", expand=True)

        top = ttk.Frame(root)
        top.pack(fill="x", pady=(0, 8))

        ttk.Label(
            top,
            text="Notas de atualização",
        ).pack(side="left")

        text_frame = ttk.Frame(root)
        text_frame.pack(fill="both", expand=True)

        scrollbar = ttk.Scrollbar(text_frame)
        scrollbar.pack(side="right", fill="y")

        self.update_notes_textbox = tk.Text(
            text_frame,
            wrap="word",
            yscrollcommand=scrollbar.set,
        )
        self.update_notes_textbox.pack(side="left", fill="both", expand=True)

        scrollbar.configure(command=self.update_notes_textbox.yview)

        self.update_notes_textbox.delete("1.0", "end")
        self.update_notes_textbox.insert("1.0", DEFAULT_UPDATE_NOTES_TEXT)
        self.update_notes_textbox.configure(state="disabled")

        self.update_notes_window.protocol(
            "WM_DELETE_WINDOW",
            self._close_update_notes_window
        )

        self.update_notes_window.lift()
        self.update_notes_window.focus_force()

    def _close_update_notes_window(self):
        if self.update_notes_window and self.update_notes_window.winfo_exists():
            try:
                self.update_notes_window.destroy()
            except Exception:
                pass

        self.update_notes_window = None
        self.update_notes_textbox = None

    def _create_log_window(self):
        self.processing_active = True
        self.processing_finished = False

        if self.log_window and self.log_window.winfo_exists():
            self.log_window.deiconify()
            self.log_window.lift()
            self.log_window.focus_force()

            if self.log_box_popup and self.log_box_popup.winfo_exists():
                self.log_box_popup.configure(state="normal")
                self.log_box_popup.delete("1.0", "end")
                self.log_box_popup.configure(state="disabled")

            if self.log_cancel_button and self.log_cancel_button.winfo_exists():
                self.log_cancel_button.configure(state="normal")

            if self.log_close_button and self.log_close_button.winfo_exists():
                self.log_close_button.configure(state="disabled")

            try:
                self.log_window.protocol(
                    "WM_DELETE_WINDOW", self._confirm_cancel_processing)
            except Exception:
                pass

            return

        self.log_window = tk.Toplevel(self)

        # remove a barra padrão do sistema (incluindo o X)
        self.log_window.overrideredirect(False)

        # mantém a janela ligada à principal
        self.log_window.transient(self)

        win_w = 900
        win_h = 430
        screen_w = self.winfo_screenwidth()
        screen_h = self.winfo_screenheight()
        pos_x = (screen_w - win_w) // 2
        pos_y = (screen_h - win_h) // 2
        self.log_window.geometry(f"{win_w}x{win_h}+{pos_x}+{pos_y}")

        outer = tk.Frame(self.log_window, bd=1, relief="solid")
        outer.pack(fill="both", expand=True)

        root = ttk.Frame(outer, padding=10)
        root.pack(fill="both", expand=True)

        top_bar = ttk.Frame(root)
        top_bar.pack(fill="x", pady=(0, 8))

        ttk.Label(
            top_bar,
            text="Log do processamento"
        ).pack(side="left")

        self.log_cancel_button = ttk.Button(
            top_bar,
            text="Cancelar",
            command=self._confirm_cancel_processing
        )
        self.log_cancel_button.pack(side="right")

        self.log_close_button = ttk.Button(
            top_bar,
            text="Fechar",
            command=self._close_finished_log_window,
            state="disabled",
        )
        self.log_close_button.pack(side="right", padx=(0, 6))

        ttk.Label(
            root,
            text="Clique em Cancelar para interromper o processamento atual.",
            anchor="w",
        ).pack(fill="x", pady=(0, 8))

        text_frame = ttk.Frame(root)
        text_frame.pack(fill="both", expand=True)

        scrollbar = ttk.Scrollbar(text_frame)
        scrollbar.pack(side="right", fill="y")

        self.log_box_popup = tk.Text(
            text_frame,
            height=20,
            wrap="word",
            state="disabled",
            yscrollcommand=scrollbar.set,
        )
        self.log_box_popup.pack(side="left", fill="both", expand=True)

        scrollbar.configure(command=self.log_box_popup.yview)

        self.log_window.protocol(
            "WM_DELETE_WINDOW", self._confirm_cancel_processing)

        try:
            self.log_window.grab_set()
        except Exception:
            pass

        self.log_window.lift()
        self.log_window.focus_force()

    def _confirm_cancel_processing(self):
        if self.processing_finished:
            self._close_finished_log_window()
            return

        parent = self.log_window if (
            self.log_window and self.log_window.winfo_exists()
        ) else self

        confirm = messagebox.askyesno(
            "Cancelar processamento",
            "Tem certeza que deseja cancelar o processamento atual?\n\n"
            "Os arquivos já processados serão mantidos.\n"
            "A interrupção pode não ser instantânea se houver uma chamada em andamento.",
            parent=parent,
        )
        if not confirm:
            return

        self.stop_event.set()

        self.log(
            "Cancelamento solicitado pelo usuário. "
            "Vou interromper assim que a etapa atual terminar."
        )

        self.set_busy(False)

        if self.log_window and self.log_window.winfo_exists():
            try:
                self.log_window.grab_release()
            except Exception:
                pass

            try:
                self.log_window.destroy()
            except Exception:
                pass

        self.processing_active = False
        self.processing_finished = False
        self.log_window = None
        self.log_box_popup = None
        self.log_cancel_button = None
        self.log_close_button = None

    def _close_finished_log_window(self):
        if self.log_window and self.log_window.winfo_exists():
            try:
                self.log_window.grab_release()
            except Exception:
                pass

            try:
                self.log_window.destroy()
            except Exception:
                pass

        self.processing_active = False
        self.processing_finished = False
        self.log_window = None
        self.log_box_popup = None
        self.log_cancel_button = None
        self.log_close_button = None

    def _set_log_window_finished_mode(self):
        def _update():
            self.processing_active = False
            self.processing_finished = True

            if self.log_cancel_button and self.log_cancel_button.winfo_exists():
                try:
                    self.log_cancel_button.configure(state="disabled")
                except Exception:
                    pass

            if self.log_close_button and self.log_close_button.winfo_exists():
                try:
                    self.log_close_button.configure(state="normal")
                except Exception:
                    pass

            if self.log_window and self.log_window.winfo_exists():
                try:
                    self.log_window.protocol(
                        "WM_DELETE_WINDOW",
                        self._close_finished_log_window
                    )
                except Exception:
                    pass

                try:
                    self.log_window.lift()
                    self.log_window.focus_force()
                except Exception:
                    pass

        self.after(0, _update)

    def _show_info_async(self, title: str, message: str):
        def _show():
            parent = self.log_window if (
                self.log_window and self.log_window.winfo_exists()
            ) else self

            messagebox.showinfo(title, message, parent=parent)

        self.after(0, _show)

    def _append_log_line(self, msg: str):
        if not (self.log_box_popup and self.log_box_popup.winfo_exists()):
            return

        self.log_box_popup.configure(state="normal")
        self.log_box_popup.insert("end", msg + "\n")
        self.log_box_popup.see("end")
        self.log_box_popup.configure(state="disabled")

    def _set_widget_state_recursive(self, parent, enabled: bool):
        desired_state = "normal" if enabled else "disabled"

        for child in parent.winfo_children():
            try:
                child.configure(state=desired_state)
            except Exception:
                try:
                    if enabled:
                        child.state(["!disabled"])
                    else:
                        child.state(["disabled"])
                except Exception:
                    pass

            self._set_widget_state_recursive(child, enabled)

    def _show_error_async(self, title: str, message: str):
        def _show():
            parent = self.log_window if (
                self.log_window and self.log_window.winfo_exists()
            ) else self

            messagebox.showerror(title, message, parent=parent)

        self.after(0, _show)

    def _raise_if_cancelled(self):
        if self.stop_event.is_set():
            raise ProcessingCancelled(
                "Processamento cancelado pelo fechamento da janela de log.")

    # ---------------- UI helpers ----------------

    def log(self, msg: str):
        self.q.put(msg)

    def _poll_queue(self):
        try:
            while True:
                msg = self.q.get_nowait()
                self._append_log_line(msg)
        except queue.Empty:
            pass
        self.after(120, self._poll_queue)

    def add_files(self):
        paths = filedialog.askopenfilenames(
            title="Selecione imagens e/ou vídeos",
            filetypes=[
                ("Mídia", "*.jpg *.jpeg *.png *.webp *.bmp *.tiff *.mp4 *.mov *.mkv *.avi *.webm *.m4v"),
                ("Todos", "*.*"),
            ],
        )
        if not paths:
            return
        for p in paths:
            if p not in self.selected_files:
                self.selected_files.append(p)
                self.files_list.insert("end", p)

    def remove_selected(self):
        sel = list(self.files_list.curselection())
        if not sel:
            return
        for idx in reversed(sel):
            path = self.files_list.get(idx)
            self.files_list.delete(idx)
            if path in self.selected_files:
                self.selected_files.remove(path)

    def clear_files(self):
        self.selected_files.clear()
        self.files_list.delete(0, "end")

    def load_roteiro(self):
        path = filedialog.askopenfilename(
            title="Selecione o roteiro.txt",
            filetypes=[("Text", "*.txt"), ("Todos", "*.*")],
        )
        if not path:
            return
        try:
            with open(path, "r", encoding="utf-8") as f:
                self.roteiro_text = f.read()
        except UnicodeDecodeError:
            with open(path, "r", encoding="latin-1") as f:
                self.roteiro_text = f.read()

        self.roteiro_info.configure(text=os.path.basename(path))

        self.roteiro_box.configure(state="normal")
        self.roteiro_box.delete("1.0", "end")
        self.roteiro_box.insert("1.0", self.roteiro_text)
        self.roteiro_box.configure(state="disabled")

    def pick_out_dir(self):
        d = filedialog.askdirectory(title="Escolha a pasta de saída")
        if d:
            self.out_dir_var.set(d)

    def set_busy(self, busy: bool):
        def _update():
            self._set_widget_state_recursive(self.main_root, not busy)

            # roteiro_box deve continuar só leitura sempre
            self.roteiro_box.configure(state="disabled")

            # menu superior também bloqueia enquanto processa
            menu_state = "disabled" if busy else "normal"
            try:
                # API do Gemini...
                self.config_menu.entryconfigure(0, state=menu_state)

                # Configurações do matching...
                self.config_menu.entryconfigure(1, state=menu_state)

                # fallback PRO
                self.config_menu.entryconfigure(3, state=menu_state)

                # vídeo com áudio
                self.config_menu.entryconfigure(4, state=menu_state)
            except Exception:
                pass

        self.after(0, _update)

    def _apply_rename_or_copy(self, src: str, dst: str, use_out_dir: bool):
        os.makedirs(os.path.dirname(dst), exist_ok=True)

        abs_src = os.path.abspath(src)
        abs_dst = os.path.abspath(dst)

        if not use_out_dir and abs_src == abs_dst:
            self.log(
                "[OK] Arquivo já estava com o nome estável. Nenhuma ação necessária.")
            return "noop"

        if use_out_dir and os.path.exists(dst):
            try:
                if os.path.isfile(dst) and _compute_content_id(src) == _compute_content_id(dst):
                    self.log(
                        "[OK] Cópia estável já existia na pasta de saída. Nenhuma ação necessária.")
                    return "noop"
            except Exception:
                pass

        if use_out_dir:
            shutil.copy2(src, dst)
            self.log("[OK] Copiado para pasta de saída.")
            return "copied"
        else:
            os.rename(src, dst)
            self.log("[OK] Renomeado no lugar.")
            return "renamed"

    def _resolve_target_path(self, src: str, directory: str, base_name: str, ext: str, use_out_dir: bool) -> str:
        desired = os.path.join(directory, base_name + ext)

        if not use_out_dir:
            if os.path.abspath(src) == os.path.abspath(desired):
                return desired
            return unique_path(directory, base_name, ext)

        if os.path.exists(desired):
            try:
                if os.path.isfile(desired) and _compute_content_id(src) == _compute_content_id(desired):
                    return desired
            except Exception:
                pass
            return unique_path(directory, base_name, ext)

        return desired

    def _handle_unassigned_scene(self, src: str):
        self.log(
            "[OK] Cena sem frase -> ignorada (não foi copiada nem renomeada).")

    def _set_progress(self, value: int):
        def _update():
            self.progress["value"] = value
        self.after(0, _update)

    def _create_placeholders_for_unmatched_phrases(
        self,
        script_items: List[str],
        matched_phrases: Set[int],
        out_dir: str,
        n_words_for_filename: int,
        selected_files: Optional[List[str]] = None,
    ) -> List[str]:
        unused = [i for i in range(len(script_items))
                  if i not in matched_phrases]
        if not unused:
            self.log("Nenhuma frase sem cena. Não vou criar placeholders.")
            return []

        if out_dir.strip():
            base_folder = out_dir
        else:
            files_base = selected_files if selected_files is not None else self.selected_files
            base_folder = os.path.dirname(
                files_base[0]) if files_base else os.getcwd()

        self.log(
            f"Frases sem cena: {len(unused)} -> criando placeholders em: {base_folder}")

        created_paths: List[str] = []

        for pi in unused:
            self._raise_if_cancelled()

            phrase = script_items[pi].strip()
            name_hint = sanitize_filename(first_n_words(
                phrase, n_words_for_filename), max_len=140)
            out_path = unique_path(base_folder, name_hint, ".png")

            create_green_placeholder_image(text=phrase, out_path=out_path)
            created_paths.append(out_path)
            self.log(f"[OK] Placeholder criado: {os.path.basename(out_path)}")

        return created_paths

    def _prune_missing_selected_files(self) -> int:
        """
        Remove da lista (UI + self.selected_files) os caminhos que não existem mais no disco.
        Retorna quantos foram removidos.
        """
        missing = [p for p in self.selected_files if (
            not os.path.exists(p)) or (not os.path.isfile(p))]
        if not missing:
            return 0

        missing_set = set(missing)

        # Reconstrói lista interna e a Listbox
        kept: List[str] = []
        self.files_list.delete(0, "end")
        for p in self.selected_files:
            if p in missing_set:
                continue
            kept.append(p)
            self.files_list.insert("end", p)

        removed_count = len(self.selected_files) - len(kept)
        self.selected_files = kept

        # Log amigável
        self.log(
            f"[WARN] {removed_count} arquivo(s) não encontrado(s) e foram removidos da lista antes do processamento.")
        for p in missing[:10]:
            self.log(f"  - {p}")
        if len(missing) > 10:
            self.log("  ...")

        return removed_count

    def _save_current_config(self):
        self._apply_runtime_matching_settings()

        save_config({
            "gemini_api_key": self.api_key_var.get().strip(),
            "words": int(self.words_var.get()),
            "out_dir": self.out_dir_var.get().strip(),
            "sentences_per_chunk": int(self.sentences_per_chunk_var.get()),
            "allow_pro_fallback": bool(self.allow_pro_var.get()),
            "include_video_audio": bool(self.include_audio_var.get()),
            "use_script_matching": bool(self.use_script_var.get()),
            "split_long_phrases": bool(self.split_long_phrases_var.get()),
            "split_words_threshold": int(self.split_words_threshold_var.get()),
            "allow_scene_reuse": bool(self.allow_scene_reuse_var.get()),
            "max_reuse_per_scene": int(self.max_reuse_per_scene_var.get()),
            "create_green_placeholders": bool(self.create_green_placeholders_var.get()),
            "min_similarity": float(self.min_similarity_var.get()),
            "min_assign_score": float(self.min_assign_score_var.get()),
            "scene_reuse_score_decay": float(self.scene_reuse_score_decay_var.get()),
            "reprocess_min_assign_score_factor": float(
                self.reprocess_min_assign_score_factor_var.get()
            ),
        })

    # ----------------- Run -----------------

    def run(self):
        if self.worker_thread and self.worker_thread.is_alive():
            messagebox.showinfo(
                "Em andamento", "Já existe um processamento em andamento.")
            return

        if not self.selected_files:
            messagebox.showerror(
                "Falta cenas", "Adicione imagens/vídeos primeiro.")
            return

        if self.use_script_var.get() and (not self.roteiro_text.strip()):
            messagebox.showerror(
                "Falta roteiro", "Abra o roteiro.txt primeiro (ou desmarque a opção de usar roteiro).")
            return

        removed = self._prune_missing_selected_files()
        if removed and not self.selected_files:
            messagebox.showerror(
                "Sem cenas", "Todos os arquivos selecionados foram removidos do disco.")
            return

        api_key = self.api_key_var.get().strip() or os.environ.get(
            "GEMINI_API_KEY", "").strip()
        if not api_key:
            messagebox.showerror(
                "Sem API key", "Defina GEMINI_API_KEY ou informe a API no menu superior.")
            return

        allow_scene_reuse = bool(self.allow_scene_reuse_var.get())
        max_reuse_per_scene = max(1, int(self.max_reuse_per_scene_var.get()))

        if allow_scene_reuse and max_reuse_per_scene > 1 and (not self.out_dir_var.get().strip()):
            messagebox.showerror(
                "Pasta de saída obrigatória",
                "Para repetir cenas, escolha uma pasta de saída.\n\n"
                "Motivo: a mesma cena pode gerar vários arquivos com nomes diferentes, "
                "então não é seguro renomear tudo no lugar."
            )
            return

        self._save_current_config()

        job = {
            "api_key": api_key,
            "n_words": int(self.words_var.get()),
            "out_dir": self.out_dir_var.get().strip(),
            "sentences_per_chunk": int(self.sentences_per_chunk_var.get()),
            "allow_pro": bool(self.allow_pro_var.get()),
            "include_audio": bool(self.include_audio_var.get()),
            "split_long": bool(self.split_long_phrases_var.get()),
            "split_limit": int(self.split_words_threshold_var.get()),
            "use_script": bool(self.use_script_var.get()),
            "roteiro_text": self.roteiro_text,
            "selected_files": list(self.selected_files),
            "allow_scene_reuse": allow_scene_reuse,
            "max_reuse_per_scene": max_reuse_per_scene,
            "create_green_placeholders": bool(self.create_green_placeholders_var.get()),
            "effective_min_assign_score": float(self.min_assign_score_var.get()),
            "reprocess_attempt_index": 0,
        }

        self.stop_event.clear()
        self._create_log_window()
        self.set_busy(True)
        self.log("Iniciando...")

        self.progress["value"] = 0
        self.progress["maximum"] = max(1, len(job["selected_files"]))

        self.worker_thread = threading.Thread(
            target=self._worker, args=(job,), daemon=True)
        self.worker_thread.start()

    def run_reprocess_pending(self):
        if self.worker_thread and self.worker_thread.is_alive():
            messagebox.showinfo(
                "Em andamento", "Já existe um processamento em andamento.")
            return

        last_state = load_last_run_state()

        if not bool(last_state.get("use_script")):
            messagebox.showerror(
                "Sem histórico útil",
                "A última execução salva não foi um processamento com roteiro."
            )
            return

        pending_ids = [
            str(x).strip()
            for x in (last_state.get("pending_scene_content_ids") or [])
            if str(x).strip()
        ]
        pending_paths = [
            str(x).strip()
            for x in (last_state.get("pending_scene_paths") or [])
            if str(x).strip()
        ]
        pending_phrase_indices = [
            int(x)
            for x in (last_state.get("pending_phrase_indices") or [])
        ]

        if not pending_ids and not pending_paths and not pending_phrase_indices:
            messagebox.showinfo(
                "Sem pendências",
                "A última execução não deixou cenas nem frases pendentes para reprocessar."
            )
            return

        saved_scene_paths = [
            str(x).strip()
            for x in (last_state.get("all_scene_paths") or [])
            if str(x).strip()
        ]

        selected_files = [
            p for p in pending_paths
            if os.path.exists(p) and os.path.isfile(p)
        ]

        if not selected_files:
            selected_files = [
                p for p in saved_scene_paths
                if os.path.exists(p) and os.path.isfile(p)
            ]

        if not selected_files:
            messagebox.showerror(
                "Sem arquivos",
                "Não encontrei os arquivos da última execução para reprocessar."
            )
            return

        roteiro_text = self.roteiro_text.strip()
        if not roteiro_text:
            roteiro_text = str(last_state.get(
                "roteiro_text", "") or "").strip()

        if not roteiro_text:
            messagebox.showerror(
                "Sem roteiro",
                "Não encontrei o roteiro atual nem o roteiro salvo da última execução."
            )
            return

        api_key = self.api_key_var.get().strip() or os.environ.get(
            "GEMINI_API_KEY", "").strip()
        if not api_key:
            messagebox.showerror(
                "Sem API key", "Defina GEMINI_API_KEY ou informe a API no menu superior.")
            return

        allow_scene_reuse = bool(self.allow_scene_reuse_var.get())
        max_reuse_per_scene = max(1, int(self.max_reuse_per_scene_var.get()))

        if allow_scene_reuse and max_reuse_per_scene > 1 and (not self.out_dir_var.get().strip()):
            messagebox.showerror(
                "Pasta de saída obrigatória",
                "Para repetir cenas, escolha uma pasta de saída antes de reprocessar pendências."
            )
            return

        self._save_current_config()

        base_min_assign_score = float(self.min_assign_score_var.get())
        last_reprocess_attempts = int(
            last_state.get("reprocess_pending_attempts", 0) or 0
        )
        reprocess_attempt_index = last_reprocess_attempts + 1

        effective_min_assign_score = max(
            float(self.min_similarity_var.get()),
            base_min_assign_score * (
                REPROCESS_MIN_ASSIGN_SCORE_FACTOR ** reprocess_attempt_index
            ),
        )

        job = {
            "api_key": api_key,
            "n_words": int(self.words_var.get()),
            "out_dir": self.out_dir_var.get().strip(),
            "sentences_per_chunk": int(self.sentences_per_chunk_var.get()),
            "allow_pro": bool(self.allow_pro_var.get()),
            "include_audio": bool(self.include_audio_var.get()),
            "split_long": bool(self.split_long_phrases_var.get()),
            "split_limit": int(self.split_words_threshold_var.get()),
            "use_script": True,
            "roteiro_text": roteiro_text,
            "selected_files": selected_files,
            "reprocess_pending_only": True,
            "allow_scene_reuse": allow_scene_reuse,
            "max_reuse_per_scene": max_reuse_per_scene,
            "create_green_placeholders": bool(self.create_green_placeholders_var.get()),
            "pending_phrase_indices": pending_phrase_indices,
            "effective_min_assign_score": effective_min_assign_score,
            "reprocess_attempt_index": reprocess_attempt_index,
        }

        self.stop_event.clear()
        self._create_log_window()
        self.set_busy(True)

        self.log("Iniciando reprocessamento de pendências...")
        self.log(
            f"[REPROCESSAR] cenas pendentes={max(len(pending_ids), len(pending_paths))} | "
            f"frases pendentes={len(pending_phrase_indices)}"
        )
        self.log(
            f"[REPROCESSAR] tentativa={reprocess_attempt_index} | "
            f"min_assign_score base={base_min_assign_score:.2f} -> efetivo={effective_min_assign_score:.2f}"
        )

        self.progress["value"] = 0
        self.progress["maximum"] = max(1, len(job["selected_files"]))

        self.worker_thread = threading.Thread(
            target=self._worker, args=(job,), daemon=True)
        self.worker_thread.start()

    def undo_last_run(self):
        if self.worker_thread and self.worker_thread.is_alive():
            messagebox.showinfo(
                "Em andamento", "Já existe um processamento em andamento.")
            return

        undo_data = load_undo_last_run()
        operations = undo_data.get("operations", [])

        if not operations:
            messagebox.showinfo(
                "Nada para desfazer",
                "Não existe uma última execução registrada para desfazer."
            )
            return

        confirm = messagebox.askyesno(
            "Desfazer última execução",
            "Isso vai tentar reverter as alterações da última execução registrada.\n\n"
            "Deseja continuar?"
        )
        if not confirm:
            return

        self.stop_event.clear()
        self._create_log_window()
        self.set_busy(True)
        self.log("Iniciando desfazer da última execução...")

        self.progress["value"] = 0
        self.progress["maximum"] = max(1, len(operations))

        self.worker_thread = threading.Thread(
            target=self._undo_worker, daemon=True)
        self.worker_thread.start()

    def _safe_remove_file_for_undo(self, path: str):
        if not path:
            return

        if os.path.exists(path) and os.path.isfile(path):
            os.remove(path)
            self.log(f"[UNDO OK] Arquivo removido: {path}")
        else:
            self.log(f"[UNDO INFO] Arquivo já não existe: {path}")

    def _undo_worker(self):
        finished_successfully = False

        try:
            undo_data = load_undo_last_run()
            operations = undo_data.get("operations", [])

            if not operations:
                self.log("Nada para desfazer.")
                finished_successfully = True
                return

            total = len(operations)

            for idx, op in enumerate(reversed(operations), start=1):
                self._raise_if_cancelled()

                op_type = str(op.get("type", "") or "").strip()
                self.log("-" * 60)
                self.log(f"[UNDO {idx}/{total}] Tipo: {op_type}")

                try:
                    if op_type == "rename":
                        src_old = str(op.get("src_old", "") or "").strip()
                        dst_new = str(op.get("dst_new", "") or "").strip()

                        if not dst_new:
                            self.log(
                                "[UNDO WARN] Operação rename sem dst_new. Ignorando.")
                        elif not os.path.exists(dst_new):
                            self.log(
                                f"[UNDO INFO] Arquivo renomeado já não existe: {dst_new}")
                        else:
                            os.makedirs(os.path.dirname(
                                src_old), exist_ok=True)
                            os.rename(dst_new, src_old)
                            self.log(
                                f"[UNDO OK] Renomeado de volta: {dst_new} -> {src_old}")

                    elif op_type == "copy":
                        copied_path = str(
                            op.get("copied_path", "") or "").strip()
                        self._safe_remove_file_for_undo(copied_path)

                    elif op_type == "placeholder":
                        placeholder_path = str(
                            op.get("path", "") or "").strip()
                        self._safe_remove_file_for_undo(placeholder_path)

                    elif op_type == "report":
                        report_path = str(op.get("path", "") or "").strip()
                        self._safe_remove_file_for_undo(report_path)

                    else:
                        self.log(
                            f"[UNDO WARN] Tipo de operação desconhecido: {op_type}")

                except Exception as e:
                    self.log(
                        f"[UNDO ERRO] Falha ao desfazer operação ({op_type}): {e}")

                self._set_progress(idx)

            clear_undo_last_run()
            self.log("=" * 60)
            self.log("Desfazer concluído. O histórico de undo foi limpo.")
            finished_successfully = True

        except ProcessingCancelled as e:
            self.log(str(e))
            self.log("O desfazer foi interrompido.")
        except Exception as e:
            self.log(f"ERRO no desfazer: {e}")
            self._show_error_async("Erro", str(e))
        finally:
            self.set_busy(False)
            self._set_log_window_finished_mode()

            if finished_successfully:
                self._show_info_async(
                    "Desfazer concluído",
                    "O desfazer foi finalizado.\n\nClique em OK para continuar."
                )

    def _worker(self, job: dict):
        finished_successfully = False

        try:
            self._raise_if_cancelled()

            api_key = job["api_key"]
            n_words = job["n_words"]
            out_dir = job["out_dir"]
            sentences_per_chunk = job["sentences_per_chunk"]
            allow_pro = job["allow_pro"]
            include_audio = job["include_audio"]
            split_long = job["split_long"]
            split_limit = job["split_limit"]
            use_script = job["use_script"]
            roteiro_text = job["roteiro_text"]
            selected_files = job["selected_files"]
            allow_scene_reuse = bool(job.get("allow_scene_reuse", False))
            max_reuse_per_scene = max(
                1, int(job.get("max_reuse_per_scene", 1)))
            reprocess_pending_only = bool(
                job.get("reprocess_pending_only", False))
            create_green_placeholders = bool(
                job.get("create_green_placeholders", True)
            )
            pending_phrase_indices = [
                int(x)
                for x in (job.get("pending_phrase_indices") or [])
            ]
            effective_min_assign_score = float(
                job.get("effective_min_assign_score", MIN_ASSIGN_SCORE)
            )
            reprocess_attempt_index = int(
                job.get("reprocess_attempt_index", 0) or 0
            )

            client = genai.Client(api_key=api_key)
            execution_undo_ops: List[dict] = []

            self.log("=" * 60)
            self.log(f"Modelo de upload: {GEMINI_VIDEO_MODEL}")
            self.log(f"Modelo de descrição: {GEMINI_IMAGE_MODEL_CHEAP}")
            self.log(
                f"Modelo de embeddings: {GEMINI_EMBEDDING_MODEL}")
            if allow_scene_reuse and max_reuse_per_scene > 1:
                self.log(
                    f"[REUSE] Repetição de cenas ativada: até {max_reuse_per_scene} uso(s) por cena | decay={SCENE_REUSE_SCORE_DECAY:.2f}"
                )

            if reprocess_pending_only:
                self.log(
                    f"[MATCHING_CFG] min_similarity={MIN_SIMILARITY:.2f} | "
                    f"min_assign_score_base={MIN_ASSIGN_SCORE:.2f} | "
                    f"min_assign_score_reprocess={effective_min_assign_score:.2f} | "
                    f"reuse_decay={SCENE_REUSE_SCORE_DECAY:.2f} | "
                    f"tentativa_reprocesso={reprocess_attempt_index}"
                )
            else:
                self.log(
                    f"[MATCHING_CFG] min_similarity={MIN_SIMILARITY:.2f} | "
                    f"min_assign_score={MIN_ASSIGN_SCORE:.2f} | "
                    f"reuse_decay={SCENE_REUSE_SCORE_DECAY:.2f}"
                )

            self.log("=" * 60)

            all_script_items: List[str] = []
            phrase_index_map: List[int] = []

            if use_script:
                all_script_items = build_script_items(
                    roteiro_text,
                    sentences_per_chunk=sentences_per_chunk,
                    split_long_phrases=split_long,
                    split_words_threshold=split_limit,
                )

                if reprocess_pending_only and pending_phrase_indices:
                    valid_pending_phrase_indices = [
                        pi for pi in pending_phrase_indices
                        if 0 <= pi < len(all_script_items)
                    ]
                    phrase_index_map = valid_pending_phrase_indices
                    script_items = [all_script_items[pi]
                                    for pi in phrase_index_map]

                    self.log(
                        f"Itens do roteiro: total={len(all_script_items)} | "
                        f"pendentes nesta rodada={len(script_items)} "
                        f"(chunk={sentences_per_chunk}, split_long={split_long}, limit={split_limit})"
                    )
                else:
                    phrase_index_map = list(range(len(all_script_items)))
                    script_items = list(all_script_items)

                    self.log(
                        f"Itens do roteiro: {len(script_items)} "
                        f"(chunk={sentences_per_chunk}, split_long={split_long}, limit={split_limit})"
                    )
            else:
                script_items = []

            if use_script:
                scene_descs = describe_all_scenes(
                    selected_files=selected_files,
                    client=client,
                    log_fn=self.log,
                    progress_fn=self._set_progress,
                    allow_pro_fallback=allow_pro,
                    include_video_audio=include_audio,
                    stop_event=self.stop_event,
                )

                self._raise_if_cancelled()

                initial_scene_use_counts = {}
                if reprocess_pending_only:
                    last_state = load_last_run_state()
                    saved_map = last_state.get(
                        "stable_scene_name_map", {}) or {}
                    for _, data in saved_map.items():
                        try:
                            scene_path = str(
                                data.get("scene_path", "") or "").strip()
                            if not scene_path:
                                continue
                            current_idx = next(
                                (i for i, s in enumerate(scene_descs)
                                 if os.path.abspath(s.path) == os.path.abspath(scene_path)),
                                None,
                            )
                            if current_idx is not None:
                                initial_scene_use_counts[current_idx] = (
                                    initial_scene_use_counts.get(
                                        current_idx, 0) + 1
                                )
                        except Exception:
                            pass

                local_assignments, local_matched_phrases, matched_scenes = build_global_assignment(
                    script_items=script_items,
                    scene_descs=scene_descs,
                    client=client,
                    log_fn=self.log,
                    max_uses_per_scene=max_reuse_per_scene if allow_scene_reuse else 1,
                    initial_scene_use_counts=initial_scene_use_counts,
                    min_similarity=float(self.min_similarity_var.get()),
                    min_assign_score=effective_min_assign_score,
                )

                assignments = [
                    Assignment(
                        phrase_idx=phrase_index_map[a.phrase_idx],
                        scene_idx=a.scene_idx,
                        score=a.score,
                    )
                    for a in local_assignments
                ]

                matched_phrases = {
                    phrase_index_map[pi]
                    for pi in local_matched_phrases
                    if 0 <= pi < len(phrase_index_map)
                }

                self.log(
                    f"Matches encontrados: {len(assignments)} (limiar={MIN_SIMILARITY}, modo=otimizacao_global_exata)"
                )

                stable_scene_name_map = {}

                if reprocess_pending_only:
                    try:
                        last_state = load_last_run_state()
                        previous_map = last_state.get(
                            "stable_scene_name_map", {}) or {}
                        if isinstance(previous_map, dict):
                            stable_scene_name_map.update(previous_map)
                    except Exception:
                        pass

                use_out_dir = bool(out_dir)
                out_base_dir = out_dir if use_out_dir else ""

                for idx, a in enumerate(assignments, start=1):
                    self._raise_if_cancelled()

                    phrase = all_script_items[a.phrase_idx]
                    scene = scene_descs[a.scene_idx]

                    self.log("-" * 60)
                    self.log(
                        f"[{idx}/{len(assignments)}] score={a.score:.3f} | cena={os.path.basename(scene.path)}"
                    )
                    self.log(f"frase: {phrase}")

                    safe_phrase = sanitize_filename(phrase)
                    short_name = first_n_words_filename(safe_phrase, n_words)
                    base_name = short_name

                    src_dir = os.path.dirname(scene.path)
                    ext = os.path.splitext(scene.path)[1]

                    target_dir = out_base_dir if use_out_dir else src_dir
                    dst = self._resolve_target_path(
                        src=scene.path,
                        directory=target_dir,
                        base_name=base_name,
                        ext=ext,
                        use_out_dir=use_out_dir,
                    )

                    action_result = self._apply_rename_or_copy(
                        src=scene.path,
                        dst=dst,
                        use_out_dir=use_out_dir,
                    )

                    if action_result == "renamed":
                        execution_undo_ops.append({
                            "type": "rename",
                            "src_old": scene.path,
                            "dst_new": dst,
                        })
                    elif action_result == "copied":
                        execution_undo_ops.append({
                            "type": "copy",
                            "copied_path": dst,
                        })

                    try:
                        stable_scene_name_map[str(a.phrase_idx)] = {
                            "phrase_text": phrase,
                            "scene_path": dst if use_out_dir else scene.path,
                            "score": float(a.score),
                        }
                    except Exception:
                        pass

                for si, s in enumerate(scene_descs):
                    if si not in matched_scenes:
                        self._handle_unassigned_scene(s.path)

                self.log("-" * 60)

                matched_phrases_for_placeholders = set(matched_phrases)

                if reprocess_pending_only:
                    try:
                        last_state = load_last_run_state()
                        previous_map = last_state.get(
                            "stable_scene_name_map", {}) or {}
                        for k in previous_map.keys():
                            try:
                                matched_phrases_for_placeholders.add(int(k))
                            except Exception:
                                pass
                    except Exception:
                        pass

                if create_green_placeholders:
                    self.log(
                        "Criando placeholders verdes para frases sem cena...")
                    created_placeholder_paths = self._create_placeholders_for_unmatched_phrases(
                        script_items=all_script_items,
                        matched_phrases=matched_phrases_for_placeholders,
                        out_dir=out_dir,
                        n_words_for_filename=n_words,
                        selected_files=selected_files,
                    )

                    for p in created_placeholder_paths:
                        execution_undo_ops.append({
                            "type": "placeholder",
                            "path": p,
                        })
                else:
                    self.log(
                        "Placeholders verdes desativados. Frases sem cena ficarão apenas no relatório."
                    )

                pending_scene_descs = [
                    s for si, s in enumerate(scene_descs)
                    if si not in matched_scenes
                ]

                pending_phrase_indices_out = sorted(
                    pi for pi in phrase_index_map
                    if pi not in matched_phrases
                )

                try:
                    save_last_run_state({
                        "created_at": datetime.utcnow().isoformat() + "Z",
                        "use_script": True,
                        "roteiro_text": roteiro_text,
                        "all_scene_paths": [s.path for s in scene_descs],
                        "stable_scene_name_map": stable_scene_name_map,
                        "pending_scene_content_ids": [
                            s.content_id for s in pending_scene_descs if s.content_id
                        ],
                        "pending_scene_paths": [s.path for s in pending_scene_descs],
                        "pending_phrase_indices": pending_phrase_indices_out,
                        "reprocess_pending_attempts": (
                            reprocess_attempt_index if reprocess_pending_only else 0
                        ),
                    })
                except Exception:
                    pass

            else:
                for idx, src in enumerate(selected_files, start=1):
                    self._raise_if_cancelled()

                    self.log("-" * 60)
                    self.log(
                        f"[{idx}/{len(selected_files)}] {os.path.basename(src)}")

                    desc = describe_single_scene(
                        src,
                        client=client,
                        log_fn=self.log,
                        allow_pro_fallback=allow_pro,
                        include_video_audio=include_audio,
                        stop_event=self.stop_event,
                    )

                    safe_desc = sanitize_filename(desc.desc_text)
                    short_name = first_n_words_filename(safe_desc, n_words)
                    ext = os.path.splitext(src)[1]

                    use_out_dir = bool(out_dir)
                    target_dir = out_dir if use_out_dir else os.path.dirname(
                        src)

                    dst = self._resolve_target_path(
                        src=src,
                        directory=target_dir,
                        base_name=short_name,
                        ext=ext,
                        use_out_dir=use_out_dir,
                    )

                    action_result = self._apply_rename_or_copy(
                        src=src,
                        dst=dst,
                        use_out_dir=use_out_dir,
                    )

                    if action_result == "renamed":
                        execution_undo_ops.append({
                            "type": "rename",
                            "src_old": src,
                            "dst_new": dst,
                        })
                    elif action_result == "copied":
                        execution_undo_ops.append({
                            "type": "copy",
                            "copied_path": dst,
                        })

                    self._set_progress(idx)

            self.log("=" * 60)
            self.log("Finalizado!")
            finished_successfully = True

        except ProcessingCancelled as e:
            self.log(str(e))
            self.log(
                "O processamento foi interrompido. Arquivos já processados antes do cancelamento foram mantidos."
            )
        except Exception as e:
            self.log(f"ERRO: {e}")
            self._show_error_async("Erro", str(e))
        finally:
            try:
                if execution_undo_ops:
                    save_undo_last_run({
                        "created_at": datetime.utcnow().isoformat() + "Z",
                        "status": "ready",
                        "operations": execution_undo_ops,
                    })
                    self.log(
                        f"[UNDO] Histórico da última execução salvo em: {UNDO_LAST_RUN_PATH}"
                    )
            except Exception:
                pass

            self.set_busy(False)
            self._set_log_window_finished_mode()

            if finished_successfully:
                self._show_info_async(
                    "Processamento concluído",
                    "O processamento foi finalizado.\n\nClique em OK para continuar."
                )


if __name__ == "__main__":
    App().mainloop()
